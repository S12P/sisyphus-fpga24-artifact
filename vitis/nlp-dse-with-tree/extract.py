    ...
    if "_evaluation" in l and "." not in l:
        ...
        for l2 in os.listdir(f"{l}/"):
            if "only_nlp" in l2 and "." not in l2:
                ...
                #     continue
                if "_done" in os.listdir(f"{l}/{l2}") and "merlin.rpt" in os.listdir(f"{l}/{l2}/_done"):
                    f = open(f"{l}/{l2}/_done/merlin.rpt", "r")
                    lines = f.readlines()
                    f.close()
                    for k, line in enumerate(lines):
                        if "Kernel" in line:
                            ...
                            if lat < mini:
                                ...
                                mdsp = dsp
                                muram = uram
                elif "done" in l2 and "merlin.rpt" in os.listdir(f"{l}/{l2}"):
                    ...
                    for k, line in enumerate(lines):
                        if "Kernel" in line:
                            ...
                            if lat < mini:
                                ...
                                mdsp = dsp
                                muram = uram
                elif "merlin.rpt" in os.listdir(f"{l}/{l2}/"):
                    f = open(f"{l}/{l2}/merlin.rpt", "r")
                    lines = f.readlines()
                    f.close()
                    for k, line in enumerate(lines):
