...
// B is MxN
{
    float B_buf[200][15][16];
/* Existing HLS partition: #pragma HLS array_partition variable=B_buf cyclic factor = 16 dim=3 */
    
#pragma HLS array_partition variable=B_buf complete dim=3
    memcpy_wide_bus_read_float_3d_15_16_512(B_buf,0,0,0,(merlin_uint_512 *)B,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
    merlinL4:
    for (i = 0; i < 50; i++) 
...
{
          memcpy_wide_bus_read_float_2d_200_256(A_6_0_buf,(size_t )0,(size_t )0,(merlin_uint_256 *)A,(size_t )((200L + ((long )(i * 804)) + ((long )(i_sub * 201))) * ((long )4)),sizeof(float ) * ((unsigned long )(39601L + ((long )(i * - 800)) + ((long )(i_sub * - 200)))),(size_t )39800L);
        }
        merlinL2:
        for (j = 0; j < 15; j++) 
...
#pragma HLS LOOP_TRIPCOUNT max=199
              B_buf[((long )i) * 4L + ((long )i_sub)][j][j_sub] += A_6_0_buf[((long )(- 1 + -i_sub)) + ((long )i) * - 4L + ((long )k)][0] * B_buf[k][j][j_sub];
            }
            B_buf[((long )i) * 4L + ((long )i_sub)][j][j_sub] = alpha * B_buf[((long )i) * 4L + ((long )i_sub)][j][j_sub];
          }
        }
      }
...
/* Existing HLS partition: #pragma HLS array_partition variable=B_buf cyclic factor = 16 dim=3 */
    memcpy_wide_bus_write_float_3d_15_16_512((merlin_uint_512 *)B,B_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )48000L),48000L);
  }
}
