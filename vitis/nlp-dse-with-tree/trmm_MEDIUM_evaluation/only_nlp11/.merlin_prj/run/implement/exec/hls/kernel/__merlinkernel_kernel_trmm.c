...
// B is MxN
{
    float B_buf[200][60][4];
/* Existing HLS partition: #pragma HLS array_partition variable=B_buf complete dim=3 */
/* Existing HLS partition: #pragma HLS array_partition variable=B_buf cyclic factor = 4 dim=2 */
    
#pragma HLS array_partition variable=B_buf complete dim=3
    
#pragma HLS array_partition variable=B_buf cyclic factor=4 dim=2
    memcpy_wide_bus_read_float_3d_60_4_512(B_buf,0,0,0,(merlin_uint_512 *)B,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
    merlinL4:
    for (i = 0; i < 25; i++) 
...
/* Existing HLS partition: #pragma HLS array_partition variable=B_buf complete dim=3 */
/* Existing HLS partition: #pragma HLS array_partition variable=B_buf cyclic factor = 4 dim=2 */
    memcpy_wide_bus_write_float_3d_60_4_512((merlin_uint_512 *)B,B_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )48000L),48000L);
  }
}
