...
{
          memcpy_wide_bus_read_float_2d_200_256(A_6_0_buf,(size_t )0,(size_t )0,(merlin_uint_256 *)A,(size_t )((200L + ((long )(i * 10050)) + ((long )(i_sub * 201))) * ((long )4)),sizeof(float ) * ((unsigned long )(39601L + ((long )(i * - 10000)) + ((long )(i_sub * - 200)))),(size_t )39800L);
        }
        merlinL4:
        for (j = 0; j < 48; j++) 
...
#pragma HLS LOOP_TRIPCOUNT max=199
              B_buf[((long )i) * 50L + ((long )i_sub)][j][j_sub] += A_6_0_buf[((long )(- 1 + -i_sub)) + ((long )i) * - 50L + ((long )k)][0] * B_buf[k][j][j_sub];
            }
            B_buf[((long )i) * 50L + ((long )i_sub)][j][j_sub] = alpha * B_buf[((long )i) * 50L + ((long )i_sub)][j][j_sub];
          }
        }
      }
