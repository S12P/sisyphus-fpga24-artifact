...
/* Existing HLS partition: #pragma HLS array_partition variable=A_6_1_buf cyclic factor = 8 dim=3 */
      memcpy_wide_bus_read_float_3d_8_200_256(A_6_1_buf,0,0,0,(merlin_uint_256 *)A,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
    }
    merlinL4:
    for (i = 0; i < 30; i++) 
...
{
        memcpy_wide_bus_read_float_2d_240_512(C_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)C,(size_t )(((long )i) * 1920L * ((long )4)),sizeof(float ) * ((unsigned long )1920L),(size_t )1920L);
      }
      merlinL3:
...
{
        memcpy_wide_bus_write_float_2d_240_512((merlin_uint_512 *)C,C_buf,(size_t )0,(size_t )0,(size_t )(((long )4) * (((long )i) * 1920L)),sizeof(float ) * ((unsigned long )1920L),(size_t )1920L);
      }
    }
