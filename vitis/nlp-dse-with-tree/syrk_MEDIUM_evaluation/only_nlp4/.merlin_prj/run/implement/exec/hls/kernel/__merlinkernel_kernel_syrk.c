...
void kernel_syrk_L_0_para_sub(float alpha,float beta,int i,int j,int k,float A_7_1_buf[200],float C_buf[120][2],float A_7_0_buf_dup[120][2][200])
...
#pragma HLS unroll
        C_buf[j][j_sub] += alpha * A_7_1_buf[k] * A_7_0_buf_dup[j][j_sub][k];
      }
    }
  }
}

void kernel_syrk_L_0_para(float alpha,float beta,int j,int k,float A_7_1_buf[240][200],float C_buf[240][120][2],float A_7_0_buf_dup[240][120][2][200])
{
...
{
      memcpy_wide_bus_read_float_3d_120_2_512(C_buf,0,0,0,(merlin_uint_512 *)C,(0 * 4),sizeof(float ) * ((unsigned long )57600L),57600L);
    }
{
      memcpy_wide_bus_read_float_3d_2_200_256(A_7_0_buf,0,0,0,(merlin_uint_256 *)A,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
/* Existing HLS partition: #pragma HLS array_partition variable=A_7_1_buf cyclic factor = 8 dim=2 */
    ...
    float A_7_0_buf_dup[240][120][2][200];
...
#pragma HLS array_partition variable=A_7_0_buf_dup complete dim=1
{
      ...
      long _i3;
      merlinL10:
      for (_i1 = ((long )0); ((unsigned long )_i1) <= 119UL; ++_i1) {
        merlinL9:
...
/* Original: #pragma ACCEL parallel */
{
      ...
      memcpy_wide_bus_write_float_3d_120_2_512((merlin_uint_512 *)C,C_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )57600L),57600L);
    }
  }
