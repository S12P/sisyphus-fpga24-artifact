...
/* Original: #pragma ACCEL PARALLEL FACTOR=120 */
{
      float B_7_1_buf[120][200];
      
#pragma HLS array_partition variable=B_7_1_buf cyclic factor=8 dim=2
      float A_7_0_buf[120][200];
      
#pragma HLS array_partition variable=A_7_0_buf cyclic factor=8 dim=2
{
        memcpy_wide_bus_read_float_2d_200_256(A_7_0_buf,(size_t )0,(size_t )0,(merlin_uint_256 *)A,(size_t )(((long )i) * 24000L * ((long )4)),sizeof(float ) * ((unsigned long )24000L),(size_t )24000L);
      }
{
        memcpy_wide_bus_read_float_2d_200_256(B_7_1_buf,(size_t )0,(size_t )0,(merlin_uint_256 *)B,(size_t )(((long )i) * 24000L * ((long )4)),sizeof(float ) * ((unsigned long )24000L),(size_t )24000L);
      }
      merlinL3:
...
#pragma HLS pipeline
          C_buf[i][i_sub][j] = compute1_1(C_buf[i][i_sub][j],beta,i * 120 + i_sub,j);
        }
        merlinL1:
        for (k = 0; k < 200; k++) {
          merlinL0:
...
#pragma HLS pipeline
            C_buf[i][i_sub][j] = compute2_1(C_buf[i][i_sub][j],A_7_1_buf[j][k],alpha,B_7_1_buf[i_sub][k],B_7_0_buf[j][k],A_7_0_buf[i_sub][k],i * 120 + i_sub,j);
          }
        }
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=A_7_0_buf cyclic factor = 8 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=B_7_1_buf cyclic factor = 8 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 16 dim=3 */
