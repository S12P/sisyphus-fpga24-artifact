...
void kernel_syr2k(float alpha,float beta,merlin_uint_256 C[7200],merlin_uint_256 A[6000],merlin_uint_256 B[6000])
{
...
#pragma HLS INTERFACE m_axi port=C offset=slave depth=7200 bundle=merlin_gmem_kernel_syr2k_256_C
...
#pragma HLS array_partition variable=A_8_0_buf cyclic factor=8 dim=2
    float C_buf[240][2][120];
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 8 dim=3 */
    
#pragma HLS array_partition variable=C_buf complete dim=3
    memcpy_wide_bus_read_float_3d_2_120_256(C_buf,0,0,0,(merlin_uint_256 *)C,(0 * 4),sizeof(float ) * ((unsigned long )57600L),57600L);
{
...
/* Existing HLS partition: #pragma HLS array_partition variable=A_8_1_buf cyclic factor = 8 dim=3 */
      memcpy_wide_bus_read_float_3d_120_200_256(A_8_1_buf,0,0,0,(merlin_uint_256 *)A,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
    }
{
      memcpy_wide_bus_read_float_3d_120_200_256(B_8_0_buf,0,0,0,(merlin_uint_256 *)B,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
...
#pragma HLS pipeline
        merlinL3:
        for (int j_sub_0 = 0; j_sub_0 < 120; ++j_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=120 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=120 */
...
#pragma HLS unroll
          C_buf[i][j][j_sub_0] = compute1_1(C_buf[i][j][j_sub_0],beta,i,j * 120 + j_sub_0);
        }
      }
      merlinL2:
      for (k = 0; k < 200; k++) {
...
#pragma HLS pipeline
          merlinL0:
          for (int j_sub = 0; j_sub < 120; ++j_sub) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=120 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=120 */
...
#pragma HLS unroll
            C_buf[i][j][j_sub] = compute2_1(C_buf[i][j][j_sub],A_8_1_buf[j][j_sub][k],alpha,B_8_1_buf[i][k],B_8_0_buf[j][j_sub][k],A_8_0_buf[i][k],i,j * 120 + j_sub);
          }
        }
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 8 dim=3 */
    memcpy_wide_bus_write_float_3d_2_120_256((merlin_uint_256 *)C,C_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )57600L),57600L);
  }
