...
#pragma HLS array_partition variable=C_buf complete dim=2
  ...
  int j;
  int k;
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf complete dim=4 */
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 8 dim=3 */
  memcpy_wide_bus_read_float_4d_5_32_2_512(C_buf,0,0,0,0,(merlin_uint_512 *)C,(0 * 4),sizeof(float ) * ((unsigned long )6400L),6400L);
{
    memcpy_wide_bus_read_float_3d_5_100_128(A_6_0_buf,0,0,0,(merlin_uint_128 *)A,(0 * 4),sizeof(float ) * ((unsigned long )10000L),10000L);
  }
{
    memcpy_wide_bus_read_float_3d_32_2_512(B_6_0_buf,0,0,0,(merlin_uint_512 *)B,(0 * 4),sizeof(float ) * ((unsigned long )6400L),6400L);
  }
  merlinL4:
  for (i = 0; i < 20; i++) 
...
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf complete dim=4 */
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 8 dim=3 */
  memcpy_wide_bus_write_float_4d_5_32_2_512((merlin_uint_512 *)C,C_buf,0,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )6400L),6400L);
}
/* Existing HLS partition: #pragma HLS array_partition variable=A_6_0_buf cyclic factor = 4 dim=3 */
/* Existing HLS partition: #pragma HLS array_partition variable=B_6_0_buf complete dim=3 */
