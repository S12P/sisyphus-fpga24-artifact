...
#pragma HLS array_partition variable=A_7_0_buf complete dim=4
  ...
  int k;
{
    memcpy_wide_bus_read_float_4d_4_25_4_128(A_7_0_buf,0,0,0,0,(merlin_uint_128 *)A,(0 * 4),sizeof(float ) * ((unsigned long )10000L),10000L);
  }
{
    memcpy_wide_bus_read_float_4d_4_4_16_512(B_7_0_buf,0,0,0,0,(merlin_uint_512 *)B,(0 * 4),sizeof(float ) * ((unsigned long )6400L),6400L);
  }
  merlinL5:
  for (i = 0; i < 25; i++) 
...
{
      memcpy_wide_bus_read_float_3d_4_16_512(C_buf,(size_t )0,(size_t )0,(size_t )0,(merlin_uint_512 *)C,(size_t )(((long )i) * 256L * ((long )4)),sizeof(float ) * ((unsigned long )256L),(size_t )256L);
    }
    merlinL4:
...
{
      memcpy_wide_bus_write_float_3d_4_16_512((merlin_uint_512 *)C,C_buf,(size_t )0,(size_t )0,(size_t )0,(size_t )(((long )4) * (((long )i) * 256L)),sizeof(float ) * ((unsigned long )256L),(size_t )256L);
    }
  }
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 16 dim=3 */
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 16 dim=3 */
}
/* Existing HLS partition: #pragma HLS array_partition variable=A_7_0_buf cyclic factor = 4 dim=4 */
