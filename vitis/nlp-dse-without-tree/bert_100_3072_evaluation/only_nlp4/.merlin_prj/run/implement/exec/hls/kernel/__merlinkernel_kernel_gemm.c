...
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 4 dim=2 */
  memcpy_wide_bus_read_float_3d_768_4_512(C_buf,0,0,0,(merlin_uint_512 *)C,(0 * 4),sizeof(float ) * ((unsigned long )307200L),307200L);
{
    memcpy_wide_bus_read_float_2d_100_128(A_5_0_buf,0,0,(merlin_uint_128 *)A,(0 * 4),sizeof(float ) * ((unsigned long )10000L),10000L);
  }
{
    memcpy_wide_bus_read_float_3d_768_4_512(B_5_0_buf,0,0,0,(merlin_uint_512 *)B,(0 * 4),sizeof(float ) * ((unsigned long )307200L),307200L);
  }
  merlinL3:
  for (i = 0; i < 100; i++) {
    merlinL2:
...
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf complete dim=3 */
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 4 dim=2 */
  memcpy_wide_bus_write_float_3d_768_4_512((merlin_uint_512 *)C,C_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )307200L),307200L);
}
