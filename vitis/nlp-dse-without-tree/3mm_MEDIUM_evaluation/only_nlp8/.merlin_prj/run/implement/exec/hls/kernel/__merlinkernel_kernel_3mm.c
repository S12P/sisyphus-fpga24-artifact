...
{
      memcpy_wide_bus_read_float_2d_200_256(A_8_0_buf,0,0,(merlin_uint_256 *)A,(0 * 4),sizeof(float ) * ((unsigned long )36000L),36000L);
    }
{
      memcpy_wide_bus_read_float_2d_190_64(B_8_0_buf,0,0,(merlin_uint_64 *)B,(0 * 4),sizeof(float ) * ((unsigned long )38000L),38000L);
    }
    merlinL8:
    for (i = 0; i < 180; i++) {
      merlinL7:
...
#pragma HLS unroll
          E_buf[i][j] += A_8_0_buf[i][k] * B_8_0_buf[k][j];
        }
      }
    }
{
