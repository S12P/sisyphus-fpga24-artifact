...
static void merlin_memcpy_2(float dst[10][18][38][5],int dst_idx_0,int dst_idx_1,int dst_idx_2,int dst_idx_3,float src[34200],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,dst_idx_2,dst_idx_3,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = (((0 * 10 + dst_idx_0) * 18 + dst_idx_1) * 38 + dst_idx_2) * 5 + dst_idx_3;
  unsigned long total_offset2 = 0 * 34200 + src_idx_0;
  merlinL2:
  for (i = 0; i < len / 4; ++i) {
...
#pragma HLS LOOP_TRIPCOUNT max=34200
    dst[(total_offset1 + i) / 5 / 38 / 18][(total_offset1 + i) / 5 / 38 % 18][(total_offset1 + i) / 5 % 38][(total_offset1 + i) % 5] = src[total_offset2 + i];
  }
}

void kernel_3mm(float E[34200],merlin_uint_64 A[18000],merlin_uint_64 B[19000],merlin_uint_64 F[19950],merlin_uint_128 C[10450],merlin_uint_64 D[23100],merlin_uint_64 G[18900])
{
      ...
      float E_buf[12][19][10];
{
        ...
        for (int i_sub_1 = 0; i_sub_1 < 12; ++i_sub_1) {
          merlin_memcpy_0(E_buf[i_sub_1],0,0,E,((long )i) * 2280L + ((long )i_sub_1) * 190L,sizeof(float ) * ((unsigned long )190L),760UL);
        }
      }
      merlinL24:
      for (int i_sub_1 = 0; i_sub_1 < 12; ++i_sub_1) 
      ...
      }
{
        merlinL19:
        for (int i_sub_1 = 0; i_sub_1 < 12; ++i_sub_1) {
          merlin_memcpy_1(E,((long )i) * 2280L + ((long )i_sub_1) * 190L,E_buf[i_sub_1],0,0,sizeof(float ) * ((unsigned long )190L),760UL);
        }
      }
    }
{
      memcpy_wide_bus_read_float_4d_10_55_4_128(C_18_0_buf,0,0,0,0,(merlin_uint_128 *)C,(0 * 4),sizeof(float ) * ((unsigned long )41800L),41800L);
    }
{
      memcpy_wide_bus_read_float_4d_4_7_30_64(D_18_0_buf,0,0,0,0,(merlin_uint_64 *)D,(0 * 4),sizeof(float ) * ((unsigned long )46200L),46200L);
    }
    merlinL18:
    for (i = 0; i < 19; i++) 
/* Original: #pragma ACCEL PARALLEL FACTOR=10 */
...
#pragma HLS array_partition variable=F_buf cyclic factor=2 dim=3
{
        ...
        for (int i_sub_0 = 0; i_sub_0 < 10; ++i_sub_0) {
          memcpy_wide_bus_read_float_2d_30_64(F_buf[i_sub_0],(size_t )0,(size_t )0,(merlin_uint_64 *)F,(size_t )((((long )i) * 2100L + ((long )i_sub_0) * 210L) * ((long )4)),sizeof(float ) * ((unsigned long )210L),(size_t )210L);
        }
      ...
      }
{
        merlinL11:
        for (int i_sub_0 = 0; i_sub_0 < 10; ++i_sub_0) {
          memcpy_wide_bus_write_float_2d_30_64((merlin_uint_64 *)F,F_buf[i_sub_0],(size_t )0,(size_t )0,(size_t )(((long )4) * (((long )i) * 2100L + ((long )i_sub_0) * 210L)),sizeof(float ) * ((unsigned long )210L),(size_t )210L);
        }
/* Existing HLS partition: #pragma HLS array_partition variable=F_buf cyclic factor = 2 dim=3 */
      }
    }
{
      merlin_memcpy_2(E_19_0_buf,0,0,0,0,E,0,sizeof(float ) * ((unsigned long )34200L),136800UL);
    }
{
      memcpy_wide_bus_read_float_4d_5_15_14_64(F_19_0_buf,0,0,0,0,(merlin_uint_64 *)F,(0 * 4),sizeof(float ) * ((unsigned long )39900L),39900L);
    }
    merlinL10:
    for (i = 0; i < 10; i++) 
/* Original: #pragma ACCEL PARALLEL FACTOR=18 */
/* Original: #pragma ACCEL PARALLEL FACTOR=18 */
{
      float G_buf[18][15][14];
      
#pragma HLS array_partition variable=G_buf cyclic factor=2 dim=3
{
        merlinL9:
        for (int i_sub = 0; i_sub < 18; ++i_sub) {
          memcpy_wide_bus_read_float_2d_14_64(G_buf[i_sub],(size_t )0,(size_t )0,(merlin_uint_64 *)G,(size_t )((((long )i) * 3780L + ((long )i_sub) * 210L) * ((long )4)),sizeof(float ) * ((unsigned long )210L),(size_t )210L);
        }
...
/* Original: #pragma ACCEL PARALLEL */
{
        merlinL7:
        for (j = 0; j < 15; j++) 
/* Original: #pragma ACCEL PARALLEL FACTOR=14 */
/* Original: #pragma ACCEL PARALLEL FACTOR=14 */
{
          merlinL6:
          for (int j_sub = 0; j_sub < 14; ++j_sub) 
/* Original: #pragma ACCEL PARALLEL FACTOR=14 */
/* Original: #pragma ACCEL PARALLEL FACTOR=14 */
/* Original: #pragma ACCEL PARALLEL */
{
      ...
      }
{
        merlinL3:
        for (int i_sub = 0; i_sub < 18; ++i_sub) {
          memcpy_wide_bus_write_float_2d_14_64((merlin_uint_64 *)G,G_buf[i_sub],(size_t )0,(size_t )0,(size_t )(((long )4) * (((long )i) * 3780L + ((long )i_sub) * 210L)),sizeof(float ) * ((unsigned long )210L),(size_t )210L);
        }
/* Existing HLS partition: #pragma HLS array_partition variable=G_buf cyclic factor = 2 dim=3 */
      }
    }
