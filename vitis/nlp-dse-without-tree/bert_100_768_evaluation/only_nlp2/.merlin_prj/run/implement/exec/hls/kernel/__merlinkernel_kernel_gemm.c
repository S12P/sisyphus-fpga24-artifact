...
void kernel_gemm(merlin_uint_128 C[19200],merlin_uint_128 A[2500],merlin_uint_128 B[19200])
{
  
#pragma HLS INTERFACE m_axi port=A offset=slave depth=2500 bundle=merlin_gmem_kernel_gemm_128_0
  
#pragma HLS INTERFACE m_axi port=B offset=slave depth=19200 bundle=merlin_gmem_kernel_gemm_128_1
...
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 4 dim=3 */
  memcpy_wide_bus_read_float_3d_64_12_128(C_buf,0,0,0,(merlin_uint_128 *)C,(0 * 4),sizeof(float ) * ((unsigned long )76800L),76800L);
{
    memcpy_wide_bus_read_float_2d_100_128(A_5_0_buf,0,0,(merlin_uint_128 *)A,(0 * 4),sizeof(float ) * ((unsigned long )10000L),10000L);
  }
{
    memcpy_wide_bus_read_float_3d_64_12_128(B_5_0_buf,0,0,0,(merlin_uint_128 *)B,(0 * 4),sizeof(float ) * ((unsigned long )76800L),76800L);
  }
  merlinL3:
  for (i = 0; i < 100; i++) {
    merlinL2:
...
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 4 dim=3 */
  memcpy_wide_bus_write_float_3d_64_12_128((merlin_uint_128 *)C,C_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )76800L),76800L);
}
/* Existing HLS partition: #pragma HLS array_partition variable=A_5_0_buf cyclic factor = 4 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=B_5_0_buf cyclic factor = 4 dim=3 */
