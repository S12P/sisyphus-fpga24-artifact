...
static void merlin_memcpy_0(float dst[60][4][15],int dst_idx_0,int dst_idx_1,int dst_idx_2,float src[3600],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,dst_idx_2,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = ((0 * 60 + dst_idx_0) * 4 + dst_idx_1) * 15 + dst_idx_2;
  unsigned long total_offset2 = 0 * 3600 + src_idx_0;
  merlinL0:
  for (i = 0; i < len / 4; ++i) {
    
#pragma HLS PIPELINE II=1
    
#pragma HLS LOOP_TRIPCOUNT max=3600
    dst[(total_offset1 + i) / 15 / 4][(total_offset1 + i) / 15 % 4][(total_offset1 + i) % 15] = src[total_offset2 + i];
  }
}

void kernel_doitgen(merlin_uint_128 A[30000],float C4[3600],merlin_uint_512 sum[4])
{
  
#pragma HLS INTERFACE m_axi port=A offset=slave depth=30000 bundle=merlin_gmem_kernel_doitgen_128_A
  
#pragma HLS INTERFACE m_axi port=C4 offset=slave depth=3600 bundle=merlin_gmem_kernel_doitgen_32_0
...
{
      merlin_memcpy_0(C4_8_0_buf,0,0,0,C4,0,sizeof(float ) * ((unsigned long )3600L),14400UL);
    }
    merlinL7:
    for (r = 0; r < 50; r++) {
      merlinL6:
...
#pragma HLS pipeline
          merlinL4:
          for (int p_sub_0 = 0; p_sub_0 < 15; ++p_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=15 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=15 */
...
#pragma HLS unroll
            sum_buf[p * 15 + p_sub_0] = ((float )0.0);
            merlinL3:
            for (s = 0; s < 60; s++) 
...
#pragma HLS unroll
              sum_buf[p * 15 + p_sub_0] += A_buf[r][q][s] * C4_8_0_buf[s][p][p_sub_0];
            }
          }
        }
        merlinL2:
