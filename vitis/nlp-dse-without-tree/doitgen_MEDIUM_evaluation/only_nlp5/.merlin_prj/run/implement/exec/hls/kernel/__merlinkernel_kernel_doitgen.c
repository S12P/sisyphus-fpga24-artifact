...
{
      memcpy_wide_bus_read_float_3d_15_4_128(C4_8_0_buf,0,0,0,(merlin_uint_128 *)C4,(0 * 4),sizeof(float ) * ((unsigned long )3600L),3600L);
    }
    merlinL6:
    for (r = 0; r < 50; r++) {
      merlinL5:
...
#pragma HLS unroll
              sum_buf[p * 4 + p_sub_0] += A_buf[r][q][s] * C4_8_0_buf[s][p][p_sub_0];
            }
          }
        }
        merlinL1:
