...
void kernel_doitgen(merlin_uint_128 A[30000],merlin_uint_64 C4[1800],merlin_uint_512 sum[4])
{
...
{
      memcpy_wide_bus_read_float_3d_10_6_64(C4_8_0_buf,0,0,0,(merlin_uint_64 *)C4,(0 * 4),sizeof(float ) * ((unsigned long )3600L),3600L);
    }
    merlinL6:
    for (r = 0; r < 50; r++) {
      merlinL5:
...
#pragma HLS pipeline
          merlinL3:
          for (int p_sub_0 = 0; p_sub_0 < 6; ++p_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=6 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=6 */
...
#pragma HLS unroll
              sum_buf[p * 6 + p_sub_0] += A_buf[r][q][s] * C4_8_0_buf[s][p][p_sub_0];
            }
          }
        }
        merlinL1:
