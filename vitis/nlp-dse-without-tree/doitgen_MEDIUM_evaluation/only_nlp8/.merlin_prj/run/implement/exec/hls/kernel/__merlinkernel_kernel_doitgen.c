...
#pragma HLS array_partition variable=A_buf complete dim=4
    memcpy_wide_bus_read_float_4d_40_2_30_64(A_buf,0,0,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )120000L),120000L);
...
{
      memcpy_wide_bus_read_float_3d_30_60_128(C4_8_0_buf,0,0,0,(merlin_uint_128 *)C4,(0 * 4),sizeof(float ) * ((unsigned long )3600L),3600L);
    }
    merlinL6:
    for (r = 0; r < 50; r++) {
      merlinL5:
...
#pragma HLS pipeline
            merlinL2:
            for (int s_sub = 0; s_sub < 30; ++s_sub) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=30 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=30 */
...
#pragma HLS unroll
            A_buf[r][q][p][p_sub] = sum_buf[p * 30 + p_sub];
          }
        }
      }
    }
...
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf cyclic factor = 2 dim=4 */
    memcpy_wide_bus_write_float_4d_40_2_30_64((merlin_uint_64 *)A,A_buf,0,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )120000L),120000L);
  }
