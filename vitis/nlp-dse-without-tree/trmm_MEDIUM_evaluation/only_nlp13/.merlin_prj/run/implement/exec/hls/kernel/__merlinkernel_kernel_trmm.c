...
{
        memcpy_wide_bus_read_float_2d_200_256(A_5_0_buf,(size_t )0,(size_t )0,(merlin_uint_256 *)A,(size_t )((200L + ((long )(i * 201))) * ((long )4)),sizeof(float ) * ((unsigned long )(39601L + ((long )(i * - 200)))),(size_t )39800L);
      }
      merlinL2:
      for (j = 0; j < 15; j++) 
...
#pragma HLS LOOP_TRIPCOUNT max=199
            B_buf[i][j][j_sub] += A_5_0_buf[- 1 + -i + k][0] * B_buf[k][j][j_sub];
          }
          B_buf[i][j][j_sub] = alpha * B_buf[i][j][j_sub];
        }
      }
    }
