...
{
      memcpy_wide_bus_read_float_3d_55_4_128(B_8_0_buf,0,0,0,(merlin_uint_128 *)B,(0 * 4),sizeof(float ) * ((unsigned long )52800L),52800L);
    }
    merlinL8:
    for (i = 0; i < 25; i++) 
...
#pragma HLS array_partition variable=C_buf cyclic factor=4 dim=2
{
        ...
        for (int i_sub = 0; i_sub < 8; ++i_sub) {
          memcpy_wide_bus_read_float_2d_4_512(C_buf[i_sub],(size_t )0,(size_t )0,(merlin_uint_512 *)C,(size_t )((((long )i) * 1760L + ((long )i_sub) * 220L) * ((long )4)),sizeof(float ) * ((unsigned long )220L),(size_t )220L);
        }
      ...
      }
{
        merlinL0:
        for (int i_sub = 0; i_sub < 8; ++i_sub) {
          memcpy_wide_bus_write_float_2d_4_512((merlin_uint_512 *)C,C_buf[i_sub],(size_t )0,(size_t )0,(size_t )(((long )4) * (((long )i) * 1760L + ((long )i_sub) * 220L)),sizeof(float ) * ((unsigned long )220L),(size_t )220L);
        }
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf complete dim=3 */
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 4 dim=2 */
      }
...
/* Existing HLS partition: #pragma HLS array_partition variable=B_8_0_buf cyclic factor = 4 dim=3 */
}
