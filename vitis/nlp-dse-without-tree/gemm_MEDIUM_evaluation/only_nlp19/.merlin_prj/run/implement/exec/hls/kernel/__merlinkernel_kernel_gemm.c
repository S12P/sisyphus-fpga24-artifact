...
#pragma HLS array_partition variable=B_6_0_buf cyclic factor=4 dim=2
    float A_6_0_buf[25][8][240];
    
#pragma HLS array_partition variable=A_6_0_buf cyclic factor=16 dim=3
{
      memcpy_wide_bus_read_float_3d_8_240_512(A_6_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
    }
{
...
#pragma HLS array_partition variable=C_buf cyclic factor=16 dim=2
{
        ...
        for (int i_sub = 0; i_sub < 8; ++i_sub) {
          memcpy_wide_bus_read_float_512(C_buf[((long )i_sub) - 0L],(merlin_uint_512 *)C,(size_t )((((long )i) * 1760L + ((long )i_sub) * 220L) * ((long )4)),sizeof(float ) * ((unsigned long )220L),(size_t )220L);
        }
      ...
      }
{
        merlinL0:
        for (int i_sub = 0; i_sub < 8; ++i_sub) {
          memcpy_wide_bus_write_float_512((merlin_uint_512 *)C,C_buf[((long )i_sub) - 0L],(size_t )(((long )4) * (((long )i) * 1760L + ((long )i_sub) * 220L)),sizeof(float ) * ((unsigned long )220L),(size_t )220L);
        }
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 16 dim=2 */
      }
    }
