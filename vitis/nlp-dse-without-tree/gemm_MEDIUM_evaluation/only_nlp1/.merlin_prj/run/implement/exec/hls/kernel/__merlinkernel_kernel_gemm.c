...
void kernel_gemm_L_1_para_sub(float alpha,float beta,int i,int j,int k,float A_7_0_buf[240],float C_buf[20][11],float B_7_0_buf_dup[240][20][11])
...
#pragma HLS pipeline
    merlinL6:
    for (int j_sub_0 = 0; j_sub_0 < 11; ++j_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=11 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=11 */
...
#pragma HLS pipeline
      merlinL3:
      for (int j_sub = 0; j_sub < 11; ++j_sub) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=11 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=11 */
...
void kernel_gemm_L_1_para(float alpha,float beta,int j,int k,float A_7_0_buf[200][240],float C_buf[200][20][11],float B_7_0_buf_dup[200][240][20][11])
{
...
static void merlin_memcpy_0(float dst[20][11],int dst_idx_1,int dst_idx_2,float src[44000],long src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_1,dst_idx_2,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = (0 * 20 + dst_idx_1) * 11 + dst_idx_2;
...
#pragma HLS LOOP_TRIPCOUNT max=220
    dst[(total_offset1 + i) / 11][(total_offset1 + i) % 11] = src[total_offset2 + i];
  }
}

static void merlin_memcpy_1(float dst[240][20][11],int dst_idx_0,int dst_idx_1,int dst_idx_2,float src[52800],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,dst_idx_2,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = ((0 * 240 + dst_idx_0) * 20 + dst_idx_1) * 11 + dst_idx_2;
...
#pragma HLS LOOP_TRIPCOUNT max=52800
    dst[(total_offset1 + i) / 11 / 20][(total_offset1 + i) / 11 % 20][(total_offset1 + i) % 11] = src[total_offset2 + i];
  }
}

static void merlin_memcpy_2(float dst[44000],long dst_idx_0,float src[20][11],int src_idx_1,int src_idx_2,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,src_idx_1,src_idx_2
  ...
  unsigned long total_offset2 = (0 * 20 + src_idx_1) * 11 + src_idx_2;
  merlinL8:
  for (i = 0; i < len / 4; ++i) {
...
#pragma HLS LOOP_TRIPCOUNT max=220
    dst[total_offset1 + i] = src[(total_offset2 + i) / 11][(total_offset2 + i) % 11];
  }
}
    ...
    }
{
    ...
    float B_7_0_buf_dup[200][240][20][11];
...
#pragma HLS array_partition variable=B_7_0_buf_dup complete dim=1
{
      ...
      for (_i1 = ((long )0); ((unsigned long )_i1) <= 239UL; ++_i1) {
        merlinL13:
        for (_i2 = ((long )0); ((unsigned long )_i2) <= 19UL; ++_i2) {
          merlinL12:
          for (_i3 = ((long )0); ((unsigned long )_i3) <= 10UL; ++_i3) {
