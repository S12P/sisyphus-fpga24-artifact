...
void kernel_gemm(float alpha,float beta,merlin_uint_128 C[11000],merlin_uint_64 A[24000],merlin_uint_128 B[13200])
{
  
#pragma HLS INTERFACE m_axi port=A offset=slave depth=24000 bundle=merlin_gmem_kernel_gemm_64_0
...
{
      memcpy_wide_bus_read_float_3d_24_10_64(A_6_0_buf,0,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
    }
{
      memcpy_wide_bus_read_float_3d_10_220_128(B_6_0_buf,0,0,0,(merlin_uint_128 *)B,(0 * 4),sizeof(float ) * ((unsigned long )52800L),52800L);
    }
    merlinL4:
    for (i = 0; i < 200; i++) {
      merlinL3:
