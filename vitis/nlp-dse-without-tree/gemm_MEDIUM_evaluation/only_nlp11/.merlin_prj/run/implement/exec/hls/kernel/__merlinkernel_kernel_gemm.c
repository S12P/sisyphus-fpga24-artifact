...
#pragma HLS array_partition variable=B_8_0_buf complete dim=3
    float A_8_0_buf[8][25][240];
    
#pragma HLS array_partition variable=A_8_0_buf cyclic factor=16 dim=3
{
      memcpy_wide_bus_read_float_3d_25_240_512(A_8_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
    }
{
...
#pragma HLS array_partition variable=C_buf complete dim=3
{
        ...
        for (int i_sub = 0; i_sub < 25; ++i_sub) {
          merlin_memcpy_1(C_buf[i_sub],0,0,C,((long )i) * 5500L + ((long )i_sub) * 220L,sizeof(float ) * ((unsigned long )220L),880UL);
        }
      }
      merlinL9:
      for (int i_sub = 0; i_sub < 25; ++i_sub) 
      ...
      }
{
        merlinL3:
        for (int i_sub = 0; i_sub < 25; ++i_sub) {
          merlin_memcpy_2(C,((long )i) * 5500L + ((long )i_sub) * 220L,C_buf[i_sub],0,0,sizeof(float ) * ((unsigned long )220L),880UL);
        }
      }
    }
  }
