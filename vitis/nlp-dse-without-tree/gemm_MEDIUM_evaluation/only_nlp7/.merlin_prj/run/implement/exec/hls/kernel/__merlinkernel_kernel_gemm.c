...
void kernel_gemm_L_0_1_para_sub(float alpha,float beta,int j,int k,float A_8_0_buf[240],float C_buf[44][5],int i_sub,float B_8_0_buf_dup[240][44][5])
...
#pragma HLS unroll
        C_buf[j][j_sub] += alpha * A_8_0_buf[k] * B_8_0_buf_dup[k][j][j_sub];
      }
    }
  }
}

void kernel_gemm_L_0_1_para(float alpha,float beta,int j,int k,float A_8_0_buf[100][240],float C_buf[100][44][5],float B_8_0_buf_dup[100][240][44][5])
{
  
#pragma HLS inline off
  int i_sub;
  merlinL2:
  for (i_sub = 0; i_sub < 100; ++i_sub) {
    
#pragma HLS unroll complete
    kernel_gemm_L_0_1_para_sub(alpha,beta,j,k,A_8_0_buf[i_sub],C_buf[i_sub],i_sub,B_8_0_buf_dup[i_sub]);
  }
}
    ...
    float B_8_0_buf[240][44][5];
{
    ...
    float B_8_0_buf_dup[100][240][44][5];
/* Existing HLS partition: #pragma HLS array_partition variable=B_8_0_buf_dup dim=1 complete */
    
#pragma HLS array_partition variable=B_8_0_buf_dup complete dim=4
    
#pragma HLS array_partition variable=B_8_0_buf_dup complete dim=1
{
      long _i0;
      long _i1;
      long _i2;
...
#pragma HLS loop_flatten
            merlinL13:
            for (_i0 = ((long )0); ((unsigned long )_i0) <= 99UL; ++_i0) {
              
#pragma HLS unroll
              B_8_0_buf_dup[_i0][_i1][_i2][_i3] = B_8_0_buf[_i1][_i2][_i3];
            }
          }
        }
      }
...
#pragma HLS array_partition variable=C_buf complete dim=1
{
        ...
        for (int i_sub = 0; i_sub < 100; ++i_sub) {
          merlin_memcpy_1(C_buf[i_sub],0,0,C,((long )i) * 22000L + ((long )i_sub) * 220L,sizeof(float ) * ((unsigned long )220L),880UL);
        }
      }
{
        memcpy_wide_bus_read_float_2d_240_512(A_8_0_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)A,(size_t )(((long )i) * 24000L * ((long )4)),sizeof(float ) * ((unsigned long )24000L),(size_t )24000L);
      }
      kernel_gemm_L_0_1_para(alpha,beta,j,k,A_8_0_buf,C_buf,B_8_0_buf_dup);
      merlinL10:
      for (int i_sub = 0; i_sub < 100; ++i_sub) 
/* Original: #pragma ACCEL PARALLEL FACTOR=100 */
/* Original: #pragma ACCEL PARALLEL FACTOR=100 */
/* Original: #pragma ACCEL PARALLEL */
{
        ...
        merlinL9:
        for (int i_sub = 0; i_sub < 100; ++i_sub) {
          merlin_memcpy_2(C,((long )i) * 22000L + ((long )i_sub) * 220L,C_buf[i_sub],0,0,sizeof(float ) * ((unsigned long )220L),880UL);
        }
      }
    }
