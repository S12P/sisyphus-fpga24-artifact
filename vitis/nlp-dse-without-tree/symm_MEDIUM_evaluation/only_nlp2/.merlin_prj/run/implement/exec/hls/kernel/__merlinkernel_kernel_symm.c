...
{
    float B_4_0_buf[200][120][2];
    
#pragma HLS array_partition variable=B_4_0_buf complete dim=3
    
#pragma HLS array_partition variable=B_4_0_buf cyclic factor=8 dim=2
    float A_5_1_buf[200][200];
    
#pragma HLS array_partition variable=A_5_1_buf complete dim=2
    float A_5_0_buf[200][200];
    
#pragma HLS array_partition variable=A_5_0_buf complete dim=2
    float B_5_1_buf[200][120][2];
    
#pragma HLS array_partition variable=B_5_1_buf complete dim=3
    
#pragma HLS array_partition variable=B_5_1_buf cyclic factor=8 dim=2
    float B_5_0_buf[199][120][2];
...
#pragma HLS array_partition variable=C_buf complete dim=1
    memcpy_wide_bus_read_float_3d_120_2_512(C_buf,0,0,0,(merlin_uint_512 *)C,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
{
      memcpy_wide_bus_read_float_3d_120_2_512(B_5_0_buf,0,0,0,(merlin_uint_512 *)B,(0 * 4),sizeof(float ) * ((unsigned long )47760L),47760L);
/* Existing HLS partition: #pragma HLS array_partition variable=B_5_1_buf complete dim=3 */
/* Existing HLS partition: #pragma HLS array_partition variable=B_5_1_buf cyclic factor = 8 dim=2 */
      memcpy_wide_bus_read_float_3d_120_2_512(B_5_1_buf,0,0,0,(merlin_uint_512 *)B,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
/* Existing HLS partition: #pragma HLS array_partition variable=B_4_0_buf complete dim=3 */
/* Existing HLS partition: #pragma HLS array_partition variable=B_4_0_buf cyclic factor = 8 dim=2 */
      memcpy_wide_bus_read_float_3d_120_2_512(B_4_0_buf,0,0,0,(merlin_uint_512 *)B,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
    }
{
      memcpy_wide_bus_read_float_2d_200_256(A_5_0_buf,0,0,(merlin_uint_256 *)A,(0 * 4),sizeof(float ) * ((unsigned long )39999L),39999L);
/* Existing HLS partition: #pragma HLS array_partition variable=A_5_1_buf cyclic factor = 8 dim=2 */
      memcpy_wide_bus_read_float_2d_200_256(A_5_1_buf,0,0,(merlin_uint_256 *)A,(0 * 4),sizeof(float ) * ((unsigned long )39999L),39999L);
    }
    merlinL3:
    for (i = 0; i < 200; i++) {
      float A_4_0_buf[1];
{
        memcpy((void *)(&A_4_0_buf[0]),(const void *)(&A[((long )i) * 201L]),sizeof(float ) * ((unsigned long )1L));
      }
      merlinL2:
      for (j = 0; j < 120; j++) 
...
// C[k][j] += alpha * B[i][j] * A[i][k];
            C_buf[k][j][j_sub] = compute1_1(C_buf[k][j][j_sub],alpha,B_5_1_buf[i][j][j_sub],A_5_1_buf[i][k],i,k);
// temp2 += B[k][j] * A[i][k];
            temp2 = compute2_1(temp2,B_5_0_buf[k][j][j_sub],A_5_0_buf[i][k],i,k);
          }
          C_buf[i][j][j_sub] = beta * C_buf[i][j][j_sub] + alpha * B_4_0_buf[i][j][j_sub] * A_4_0_buf[0] + alpha * temp2;
        }
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf complete dim=3 */
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 8 dim=2 */
    memcpy_wide_bus_write_float_3d_120_2_512((merlin_uint_512 *)C,C_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )48000L),48000L);
  }
