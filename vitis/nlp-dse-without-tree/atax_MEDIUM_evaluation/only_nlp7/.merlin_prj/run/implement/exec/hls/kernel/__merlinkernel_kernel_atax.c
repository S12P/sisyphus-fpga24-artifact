...
static void merlin_memcpy_0(float dst[10][41],int dst_idx_0,int dst_idx_1,float src[410],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = (0 * 10 + dst_idx_0) * 41 + dst_idx_1;
...
#pragma HLS LOOP_TRIPCOUNT max=410
    dst[(total_offset1 + i) / 41][(total_offset1 + i) % 41] = src[total_offset2 + i];
  }
}

static void merlin_memcpy_1(float dst[390][10][41],int dst_idx_0,int dst_idx_1,int dst_idx_2,float src[159900],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,dst_idx_2,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = ((0 * 390 + dst_idx_0) * 10 + dst_idx_1) * 41 + dst_idx_2;
...
#pragma HLS LOOP_TRIPCOUNT max=159900
    dst[(total_offset1 + i) / 41 / 10][(total_offset1 + i) / 41 % 10][(total_offset1 + i) % 41] = src[total_offset2 + i];
  }
}

static void merlin_memcpy_2(float dst[390][10][41],int dst_idx_0,int dst_idx_1,int dst_idx_2,float src[159900],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
static void merlin_memcpy_3(float dst[10][41],int dst_idx_0,int dst_idx_1,float src[410],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
static void merlin_memcpy_4(float dst[410],int dst_idx_0,float src[10][41],int src_idx_0,int src_idx_1,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,src_idx_0,src_idx_1
  ...
  unsigned long total_offset2 = (0 * 10 + src_idx_0) * 41 + src_idx_1;
  merlinL4:
  for (i = 0; i < len / 4; ++i) {
...
#pragma HLS LOOP_TRIPCOUNT max=410
    dst[total_offset1 + i] = src[(total_offset2 + i) / 41][(total_offset2 + i) % 41];
  }
}
...
/* Original: #pragma ACCEL PARALLEL FACTOR=41 */
...
#pragma HLS pipeline
      merlinL10:
      for (int i_sub = 0; i_sub < 41; ++i_sub) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=41 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=41 */
...
#pragma HLS pipeline
        merlinL7:
        for (int j_sub_0 = 0; j_sub_0 < 41; ++j_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=41 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=41 */
...
#pragma HLS pipeline
        merlinL5:
        for (int j_sub = 0; j_sub < 41; ++j_sub) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=41 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=41 */
