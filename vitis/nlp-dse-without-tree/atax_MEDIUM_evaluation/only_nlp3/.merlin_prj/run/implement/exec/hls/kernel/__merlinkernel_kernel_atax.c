...
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf cyclic factor = 8 dim=1 */
    memcpy_wide_bus_read_float_2d_2_512(tmp_buf,0,0,(merlin_uint_512 *)tmp,(0 * 4),sizeof(float ) * ((unsigned long )390),390L);
{
      memcpy_wide_bus_read_float_3d_2_410_64(A_5_0_buf,0,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )159900L),159900L);
    }
{
...
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf cyclic factor = 8 dim=1 */
    memcpy_wide_bus_write_float_2d_2_512((merlin_uint_512 *)tmp,tmp_buf,0,0,(4 * 0),sizeof(float ) * ((unsigned long )390),390L);
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf cyclic factor = 16 dim=1 */
