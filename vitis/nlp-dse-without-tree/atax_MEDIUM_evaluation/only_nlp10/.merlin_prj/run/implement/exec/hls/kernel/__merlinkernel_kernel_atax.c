...
{
      memcpy_wide_bus_read_float_2d_410_64(A_5_0_buf,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )159900L),159900L);
...
#pragma HLS pipeline
        y_buf[j] = y_buf[j] + A_5_0_buf[i][j] * tmp_buf[i];
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf cyclic factor = 16 dim=1 */
