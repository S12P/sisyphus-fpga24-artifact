...
#pragma HLS array_partition variable=y_buf complete dim=2
    memcpy_wide_bus_read_float_2d_82_64(y_buf,0,0,(merlin_uint_64 *)y,(0 * 4),sizeof(float ) * ((unsigned long )410),410L);
    merlinL6:
    for (i = 0; i < 5; i++) 
...
#pragma HLS pipeline
      merlinL5:
      for (int i_sub = 0; i_sub < 82; ++i_sub) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=82 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=82 */
...
/* Existing HLS partition: #pragma HLS array_partition variable=A_7_0_buf cyclic factor = 2 dim=3 */
      memcpy_wide_bus_read_float_3d_5_82_64(A_7_0_buf,0,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )159900L),159900L);
    }
{
      memcpy_wide_bus_read_float_2d_82_64(x_7_0_buf,0,0,(merlin_uint_64 *)x,(0 * 4),sizeof(float ) * ((unsigned long )410),410L);
    }
    merlinL4:
    for (i = 0; i < 390; i++) {
      tmp_buf[i] = ((float )0.0);
...
#pragma HLS pipeline
        merlinL2:
        for (int j_sub_0 = 0; j_sub_0 < 82; ++j_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=82 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=82 */
...
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf cyclic factor = 2 dim=2 */
    memcpy_wide_bus_write_float_2d_82_64((merlin_uint_64 *)y,y_buf,0,0,(4 * 0),sizeof(float ) * ((unsigned long )410),410L);
  }
