...
#pragma HLS array_partition variable=s_buf complete dim=2
    memcpy_wide_bus_read_float_2d_6_64(s_buf,0,0,(merlin_uint_64 *)s,(0 * 4),sizeof(float ) * ((unsigned long )390),390L);
    merlinL4:
    for (i = 0; i < 65; i++) 
...
{
      memcpy_wide_bus_read_float_3d_65_6_64(A_6_0_buf,0,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )159900L),159900L);
/* Existing HLS partition: #pragma HLS array_partition variable=A_6_1_buf cyclic factor = 2 dim=3 */
      memcpy_wide_bus_read_float_3d_65_6_64(A_6_1_buf,0,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )159900L),159900L);
    }
{
      memcpy_wide_bus_read_float_2d_6_64(p_6_0_buf,0,0,(merlin_uint_64 *)p,(0 * 4),sizeof(float ) * ((unsigned long )390),390L);
    }
    merlinL2:
    for (i = 0; i < 410; i++) {
      q_buf[i] = ((float )0.0);
...
/* Existing HLS partition: #pragma HLS array_partition variable=s_buf cyclic factor = 2 dim=2 */
    memcpy_wide_bus_write_float_2d_6_64((merlin_uint_64 *)s,s_buf,0,0,(4 * 0),sizeof(float ) * ((unsigned long )390),390L);
  }
