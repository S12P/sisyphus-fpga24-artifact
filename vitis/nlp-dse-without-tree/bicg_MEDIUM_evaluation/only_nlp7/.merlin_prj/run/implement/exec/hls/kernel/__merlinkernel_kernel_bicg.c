...
static void merlin_memcpy_0(float dst[10][39],int dst_idx_0,int dst_idx_1,float src[390],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = (0 * 10 + dst_idx_0) * 39 + dst_idx_1;
...
#pragma HLS LOOP_TRIPCOUNT max=390
    dst[(total_offset1 + i) / 39][(total_offset1 + i) % 39] = src[total_offset2 + i];
  }
}

static void merlin_memcpy_1(float dst[410][10][39],int dst_idx_0,int dst_idx_1,int dst_idx_2,float src[159900],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,dst_idx_2,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = ((0 * 410 + dst_idx_0) * 10 + dst_idx_1) * 39 + dst_idx_2;
...
#pragma HLS LOOP_TRIPCOUNT max=159900
    dst[(total_offset1 + i) / 39 / 10][(total_offset1 + i) / 39 % 10][(total_offset1 + i) % 39] = src[total_offset2 + i];
  }
}

static void merlin_memcpy_2(float dst[410][10][39],int dst_idx_0,int dst_idx_1,int dst_idx_2,float src[159900],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
static void merlin_memcpy_3(float dst[10][39],int dst_idx_0,int dst_idx_1,float src[390],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
static void merlin_memcpy_4(float dst[390],int dst_idx_0,float src[10][39],int src_idx_0,int src_idx_1,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,src_idx_0,src_idx_1
  ...
  unsigned long total_offset2 = (0 * 10 + src_idx_0) * 39 + src_idx_1;
  merlinL4:
  for (i = 0; i < len / 4; ++i) {
...
#pragma HLS LOOP_TRIPCOUNT max=390
    dst[total_offset1 + i] = src[(total_offset2 + i) / 39][(total_offset2 + i) % 39];
  }
}
...
#pragma HLS array_partition variable=s_buf complete dim=2
    ...
    merlinL9:
    for (i = 0; i < 10; i++) 
/* Original: #pragma ACCEL PARALLEL FACTOR=39 */
/* Original: #pragma ACCEL PARALLEL FACTOR=39 */
...
#pragma HLS pipeline
      merlinL8:
      for (int i_sub = 0; i_sub < 39; ++i_sub) 
/* Original: #pragma ACCEL PARALLEL FACTOR=39 */
/* Original: #pragma ACCEL PARALLEL FACTOR=39 */
...
#pragma HLS pipeline
        merlinL5:
        for (int j_sub = 0; j_sub < 39; ++j_sub) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=39 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=39 */
