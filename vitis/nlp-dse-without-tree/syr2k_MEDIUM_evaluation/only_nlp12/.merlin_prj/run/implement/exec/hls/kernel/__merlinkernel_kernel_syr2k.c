...
#pragma HLS array_partition variable=C_buf cyclic factor=16 dim=3
    memcpy_wide_bus_read_float_3d_16_240_512(C_buf,0,0,0,(merlin_uint_512 *)C,(0 * 4),sizeof(float ) * ((unsigned long )57600L),57600L);
{
...
/* Existing HLS partition: #pragma HLS array_partition variable=B_7_1_buf cyclic factor = 8 dim=3 */
      memcpy_wide_bus_read_float_3d_16_200_256(B_7_1_buf,0,0,0,(merlin_uint_256 *)B,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
    }
    merlinL4:
    for (i = 0; i < 15; i++) 
...
#pragma HLS pipeline
          C_buf[i][i_sub][j] = compute1_1(C_buf[i][i_sub][j],beta,i * 16 + i_sub,j);
        }
        merlinL1:
        for (k = 0; k < 200; k++) {
          merlinL0:
...
#pragma HLS pipeline
            C_buf[i][i_sub][j] = compute2_1(C_buf[i][i_sub][j],A_7_1_buf[j][k],alpha,B_7_1_buf[i][i_sub][k],B_7_0_buf[j][k],A_7_0_buf[i][i_sub][k],i * 16 + i_sub,j);
          }
        }
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=C_buf cyclic factor = 16 dim=3 */
    memcpy_wide_bus_write_float_3d_16_240_512((merlin_uint_512 *)C,C_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )57600L),57600L);
  }
