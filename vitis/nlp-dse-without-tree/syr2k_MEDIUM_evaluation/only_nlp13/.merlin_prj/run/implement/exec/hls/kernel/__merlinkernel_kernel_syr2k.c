...
{
      memcpy_wide_bus_read_float_3d_16_200_256(B_8_0_buf,0,0,0,(merlin_uint_256 *)B,(0 * 4),sizeof(float ) * ((unsigned long )48000L),48000L);
...
#pragma HLS unroll
          C_buf[i][j][j_sub_0] = compute1_1(C_buf[i][j][j_sub_0],beta,i,j * 16 + j_sub_0);
        }
      }
      merlinL2:
      for (k = 0; k < 200; k++) {
...
#pragma HLS unroll
            C_buf[i][j][j_sub] = compute2_1(C_buf[i][j][j_sub],A_8_1_buf[j][j_sub][k],alpha,B_8_1_buf[i][k],B_8_0_buf[j][j_sub][k],A_8_0_buf[i][k],i,j * 16 + j_sub);
          }
        }
      }
    }
