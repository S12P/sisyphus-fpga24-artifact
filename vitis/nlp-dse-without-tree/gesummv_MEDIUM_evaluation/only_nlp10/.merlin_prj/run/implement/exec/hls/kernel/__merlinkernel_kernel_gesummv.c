...
#pragma HLS array_partition variable=x_4_0_buf cyclic factor=16 dim=1
    float A_4_0_buf[25][10][250];
    
#pragma HLS array_partition variable=A_4_0_buf cyclic factor=2 dim=3
{
      memcpy_wide_bus_read_float_3d_10_250_64(A_4_0_buf,0,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )62500L),62500L);
    }
{
...
{
      memcpy_wide_bus_read_float_3d_10_250_64(B_4_0_buf,0,0,0,(merlin_uint_64 *)B,(0 * 4),sizeof(float ) * ((unsigned long )62500L),62500L);
    }
    merlinL2:
    for (i = 0; i < 25; i++) 
...
{
        memcpy_wide_bus_read_float_2d_1_512(tmp_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)tmp,(size_t )(i * 10 * 4),sizeof(float ) * ((unsigned long )10),(size_t )10L);
      }
{
        memcpy_wide_bus_read_float_2d_1_512(y_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)y,(size_t )(i * 10 * 4),sizeof(float ) * ((unsigned long )10),(size_t )10L);
      }
      merlinL1:
...
{
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)y,y_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 10)),sizeof(float ) * ((unsigned long )10),(size_t )10L);
      }
{
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)tmp,tmp_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 10)),sizeof(float ) * ((unsigned long )10),(size_t )10L);
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf complete dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf complete dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf cyclic factor = 1 dim=0 */
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf cyclic factor = 1 dim=0 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf complete dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf complete dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf cyclic factor = 1 dim=0 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf cyclic factor = 1 dim=0 */
  }
