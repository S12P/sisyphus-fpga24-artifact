...
{
        memcpy_wide_bus_read_float_2d_1_512(tmp_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)tmp,(size_t )(i * 5 * 4),sizeof(float ) * ((unsigned long )5),(size_t )5L);
      }
{
        memcpy_wide_bus_read_float_2d_1_512(y_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)y,(size_t )(i * 5 * 4),sizeof(float ) * ((unsigned long )5),(size_t )5L);
      }
      merlinL1:
...
{
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)y,y_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 5)),sizeof(float ) * ((unsigned long )5),(size_t )5L);
      }
{
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)tmp,tmp_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 5)),sizeof(float ) * ((unsigned long )5),(size_t )5L);
      }
    }
...
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf complete dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf cyclic factor = 3 dim=0 */
/* Existing HLS partition: #pragma HLS array_partition variable=tmp_buf cyclic factor = 3 dim=0 */
...
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf complete dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf cyclic factor = 3 dim=0 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_buf cyclic factor = 3 dim=0 */
  }
