...
{
      memcpy_wide_bus_read_float_3d_5_50_64(A_4_0_buf,0,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )62500L),62500L);
    }
{
      memcpy_wide_bus_read_float_2d_50_64(x_4_0_buf,0,0,(merlin_uint_64 *)x,(0 * 4),sizeof(float ) * ((unsigned long )250),250L);
/* Existing HLS partition: #pragma HLS array_partition variable=x_4_1_buf cyclic factor = 2 dim=2 */
      memcpy_wide_bus_read_float_2d_50_64(x_4_1_buf,0,0,(merlin_uint_64 *)x,(0 * 4),sizeof(float ) * ((unsigned long )250),250L);
    }
{
      memcpy_wide_bus_read_float_3d_5_50_64(B_4_0_buf,0,0,0,(merlin_uint_64 *)B,(0 * 4),sizeof(float ) * ((unsigned long )62500L),62500L);
    }
    merlinL2:
    for (i = 0; i < 250; i++) {
      tmp_buf[i] = ((float )0.0);
