...
{
    memcpy_wide_bus_read_float_3d_32_2_512(B_5_0_buf,0,0,0,(merlin_uint_512 *)B,(0 * 4),sizeof(float ) * ((unsigned long )6400L),6400L);
  }
  merlinL3:
  for (i = 0; i < 100; i++) {
    merlinL2:
...
{
        memcpy_wide_bus_read_float_2d_1_512(C_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)C,(size_t )((((long )i) * 64L + ((long )(j * 2))) * ((long )4)),sizeof(float ) * ((unsigned long )2L),(size_t )2L);
      }
      merlinL1:
...
{
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)C,C_buf,(size_t )0,(size_t )0,(size_t )(((long )4) * (((long )i) * 64L + ((long )(j * 2)))),sizeof(float ) * ((unsigned long )2L),(size_t )2L);
      }
    }
