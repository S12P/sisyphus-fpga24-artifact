...
static void merlin_memcpy_0(float dst[60][3][70][3],int dst_idx_0,int dst_idx_1,int dst_idx_2,int dst_idx_3,float src[37800],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,dst_idx_2,dst_idx_3,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = (((0 * 60 + dst_idx_0) * 3 + dst_idx_1) * 70 + dst_idx_2) * 3 + dst_idx_3;
...
#pragma HLS LOOP_TRIPCOUNT max=37800
    dst[(total_offset1 + i) / 3 / 70 / 3][(total_offset1 + i) / 3 / 70 % 3][(total_offset1 + i) / 3 % 70][(total_offset1 + i) % 3] = src[total_offset2 + i];
  }
}

static void merlin_memcpy_1(float dst[70][3][38][5],int dst_idx_0,int dst_idx_1,int dst_idx_2,int dst_idx_3,float src[39900],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,dst_idx_2,dst_idx_3,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = (((0 * 70 + dst_idx_0) * 3 + dst_idx_1) * 38 + dst_idx_2) * 5 + dst_idx_3;
...
static void merlin_memcpy_4(float dst[60][3][95][2],int dst_idx_0,int dst_idx_1,int dst_idx_2,int dst_idx_3,float src[34200],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,dst_idx_2,dst_idx_3,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = (((0 * 60 + dst_idx_0) * 3 + dst_idx_1) * 95 + dst_idx_2) * 2 + dst_idx_3;
  unsigned long total_offset2 = 0 * 34200 + src_idx_0;
  merlinL4:
  for (i = 0; i < len / 4; ++i) {
...
#pragma HLS LOOP_TRIPCOUNT max=34200
    dst[(total_offset1 + i) / 2 / 95 / 3][(total_offset1 + i) / 2 / 95 % 3][(total_offset1 + i) / 2 % 95][(total_offset1 + i) % 2] = src[total_offset2 + i];
  }
}

static void merlin_memcpy_5(float dst[95][2][44][5],int dst_idx_0,int dst_idx_1,int dst_idx_2,int dst_idx_3,float src[41800],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
/* Original: #pragma ACCEL PARALLEL FACTOR=3 */
{
      float tmp_buf[3][38][5];
{
        merlinL22:
        for (int i_sub_0 = 0; i_sub_0 < 3; ++i_sub_0) {
          merlin_memcpy_2(tmp_buf[i_sub_0],0,0,tmp,((long )i) * 570L + ((long )i_sub_0) * 190L,sizeof(float ) * ((unsigned long )190L),760UL);
        }
      }
      merlinL21:
      for (int i_sub_0 = 0; i_sub_0 < 3; ++i_sub_0) 
...
#pragma HLS pipeline
              merlinL17:
              for (int k_sub_0 = 0; k_sub_0 < 3; ++k_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=3 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=3 */
      ...
      }
{
        merlinL16:
        for (int i_sub_0 = 0; i_sub_0 < 3; ++i_sub_0) {
          merlin_memcpy_3(tmp,((long )i) * 570L + ((long )i_sub_0) * 190L,tmp_buf[i_sub_0],0,0,sizeof(float ) * ((unsigned long )190L),760UL);
        }
      }
    }
{
      ...
      float D_buf[3][44][5];
{
        ...
        for (int i_sub = 0; i_sub < 3; ++i_sub) {
          merlin_memcpy_6(D_buf[i_sub],0,0,D,((long )i) * 660L + ((long )i_sub) * 220L,sizeof(float ) * ((unsigned long )220L),880UL);
        }
      }
      merlinL13:
      for (int i_sub = 0; i_sub < 3; ++i_sub) 
      ...
      }
{
        merlinL8:
        for (int i_sub = 0; i_sub < 3; ++i_sub) {
          merlin_memcpy_7(D,((long )i) * 660L + ((long )i_sub) * 220L,D_buf[i_sub],0,0,sizeof(float ) * ((unsigned long )220L),880UL);
        }
      }
    }
  }
