...
static void merlin_memcpy_0(float dst[180][2][105],int dst_idx_0,int dst_idx_1,int dst_idx_2,float src[37800],int src_idx_0,unsigned int len,unsigned int max_len)
{
...
#pragma HLS function_instantiate variable=dst_idx_0,dst_idx_1,dst_idx_2,src_idx_0
  unsigned long i;
  unsigned long total_offset1 = ((0 * 180 + dst_idx_0) * 2 + dst_idx_1) * 105 + dst_idx_2;
...
#pragma HLS LOOP_TRIPCOUNT max=37800
    dst[(total_offset1 + i) / 105 / 2][(total_offset1 + i) / 105 % 2][(total_offset1 + i) % 105] = src[total_offset2 + i];
  }
}
...
{
      memcpy_wide_bus_read_float_3d_105_190_64(B_8_0_buf,0,0,0,(merlin_uint_64 *)B,(0 * 4),sizeof(float ) * ((unsigned long )39900L),39900L);
    }
    merlinL8:
    for (i = 0; i < 180; i++) {
      merlinL7:
...
/* Original: #pragma ACCEL PARALLEL FACTOR=105 */
...
#pragma HLS pipeline
          merlinL5:
          for (int k_sub_0 = 0; k_sub_0 < 105; ++k_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=105 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=105 */
...
{
      memcpy_wide_bus_read_float_3d_95_220_128(C_9_0_buf,0,0,0,(merlin_uint_128 *)C,(0 * 4),sizeof(float ) * ((unsigned long )41800L),41800L);
    }
    merlinL4:
    for (i = 0; i < 180; i++) {
      merlinL3:
...
#pragma HLS unroll
            D_buf[i][j] += tmp_buf[i][((long )k) * 95L + ((long )k_sub)] * C_9_0_buf[k][k_sub][j];
          }
        }
      }
    }
