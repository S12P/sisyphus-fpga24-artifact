...
#pragma HLS array_partition variable=tmp_buf cyclic factor=32 dim=2
    memcpy_wide_bus_read_float_2d_190_64(tmp_buf,0,0,(merlin_uint_64 *)tmp,(0 * 4),sizeof(float ) * ((unsigned long )34200L),34200L);
{
      memcpy_wide_bus_read_float_3d_5_42_64(A_8_0_buf,0,0,0,(merlin_uint_64 *)A,(0 * 4),sizeof(float ) * ((unsigned long )37800L),37800L);
    }
{
      memcpy_wide_bus_read_float_3d_42_190_64(B_8_0_buf,0,0,0,(merlin_uint_64 *)B,(0 * 4),sizeof(float ) * ((unsigned long )39900L),39900L);
    }
    merlinL7:
    for (i = 0; i < 180; i++) {
      merlinL6:
...
/* Original: #pragma ACCEL PARALLEL FACTOR=42 */
...
#pragma HLS pipeline
          merlinL4:
          for (int k_sub_0 = 0; k_sub_0 < 42; ++k_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=42 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=42 */
...
{
      memcpy_wide_bus_read_float_3d_38_220_128(C_9_0_buf,0,0,0,(merlin_uint_128 *)C,(0 * 4),sizeof(float ) * ((unsigned long )41800L),41800L);
    }
    merlinL3:
    for (i = 0; i < 180; i++) {
      merlinL2:
...
#pragma HLS unroll
            D_buf[i][j] += tmp_buf[i][((long )k) * 38L + ((long )k_sub)] * C_9_0_buf[k][k_sub][j];
          }
        }
      }
    }
