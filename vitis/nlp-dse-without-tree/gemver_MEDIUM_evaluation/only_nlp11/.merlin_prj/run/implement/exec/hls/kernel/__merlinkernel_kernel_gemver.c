...
void kernel_gemver(float alpha,float beta,merlin_uint_512 A[10000],merlin_uint_512 u1[25],merlin_uint_512 v1[25],merlin_uint_512 u2[25],merlin_uint_512 v2[25],merlin_uint_512 w[25],merlin_uint_512 x[25],merlin_uint_512 y[25],merlin_uint_128 z[100])
{
...
#pragma HLS INTERFACE m_axi port=z offset=slave depth=100 bundle=merlin_gmem_kernel_gemver_128_0
...
#pragma HLS array_partition variable=w_buf complete dim=2
    float A_buf[25][16][50][8];
    
#pragma HLS array_partition variable=A_buf complete dim=4
    
#pragma HLS array_partition variable=A_buf cyclic factor=2 dim=3
...
#pragma HLS array_partition variable=x_buf cyclic factor=100 dim=1
...
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf cyclic factor = 2 dim=3 */
    memcpy_wide_bus_read_float_4d_16_50_8_512(A_buf,0,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
{
      memcpy_wide_bus_read_float_2d_16_512(u1_13_0_buf,0,0,(merlin_uint_512 *)u1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_8_512(v1_13_0_buf,0,0,(merlin_uint_512 *)v1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_16_512(u2_13_0_buf,0,0,(merlin_uint_512 *)u2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_8_512(v2_13_0_buf,0,0,(merlin_uint_512 *)v2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL13:
    for (i = 0; i < 25; i++) 
...
#pragma HLS unroll
          merlinL10:
          for (int j_sub_1 = 0; j_sub_1 < 8; ++j_sub_1) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=8 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=8 */
...
#pragma HLS unroll
            x_buf[i * 8 + i_sub_1] = x_buf[i * 8 + i_sub_1] + beta * A_buf[j][j_sub_0][i][i_sub_1] * y_14_0_buf[j][j_sub_0];
          }
        }
      }
    }
...
#pragma HLS unroll
        x_buf[i * 100 + i_sub_0] = x_buf[i * 100 + i_sub_0] + z_8_0_buf[i][i_sub_0];
      }
    }
...
#pragma HLS unroll
            w_buf[i][i_sub] = w_buf[i][i_sub] + alpha * A_buf[i][i_sub][j][j_sub] * x_buf[j * 8 + j_sub];
          }
        }
      }
    }
...
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf complete dim=4 */
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf cyclic factor = 2 dim=3 */
    memcpy_wide_bus_write_float_4d_16_50_8_512((merlin_uint_512 *)A,A_buf,0,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )160000L),160000L);
  }
...
/* Existing HLS partition: #pragma HLS array_partition variable=v1_13_0_buf cyclic factor = 2 dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=v2_13_0_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=v2_13_0_buf cyclic factor = 2 dim=1 */
