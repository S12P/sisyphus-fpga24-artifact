...
#pragma HLS array_partition variable=u1_6_0_buf cyclic factor=16 dim=1
    float y_9_0_buf[200][2];
    
#pragma HLS array_partition variable=y_9_0_buf complete dim=2
    
#pragma HLS array_partition variable=y_9_0_buf cyclic factor=8 dim=1
    float x_10_0_buf[400];
    
#pragma HLS array_partition variable=x_10_0_buf complete dim=1
    float w_buf[200][2];
...
{
      memcpy_wide_bus_read_float_2d_2_512(y_9_0_buf,0,0,(merlin_uint_512 *)y,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL6:
    for (i = 0; i < 400; i++) 
/* Original: #pragma ACCEL PARALLEL FACTOR=400 */
...
#pragma HLS unroll
          x_buf[((long )i) - 0L][0] = x_buf[((long )i) - 0L][0] + beta * A_buf[((long )j) * 2L + ((long )j_sub)][i] * y_9_0_buf[j][j_sub];
        }
      }
    }
{
...
{
      memcpy_wide_bus_read_float_512(&x_10_0_buf[0],(merlin_uint_512 *)x,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL2:
    for (i = 0; i < 200; i++) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
...
#pragma HLS unroll
          w_buf[i][i_sub] = w_buf[i][i_sub] + alpha * A_buf[((long )i) * 2L + ((long )i_sub)][j] * x_10_0_buf[j];
        }
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=w_buf complete dim=2 */
...
/* Existing HLS partition: #pragma HLS array_partition variable=x_10_0_buf cyclic factor = 16 dim=1 */
...
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf cyclic factor = 16 dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_9_0_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_9_0_buf cyclic factor = 8 dim=1 */
