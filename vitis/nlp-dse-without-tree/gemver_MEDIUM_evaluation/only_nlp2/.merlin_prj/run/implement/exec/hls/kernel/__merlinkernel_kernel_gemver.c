...
#pragma HLS array_partition variable=y_9_0_buf complete dim=1
    float w_buf[200][2];
...
{
      memcpy_wide_bus_read_float_512(&y_9_0_buf[0],(merlin_uint_512 *)y,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL6:
    for (i = 0; i < 200; i++) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
...
#pragma HLS unroll
          x_buf[i * 2 + i_sub_0] = x_buf[i * 2 + i_sub_0] + beta * A_buf[j][((long )i) * 2L + ((long )i_sub_0)] * y_9_0_buf[j];
        }
      }
    }
{
...
/* Existing HLS partition: #pragma HLS array_partition variable=y_9_0_buf cyclic factor = 16 dim=1 */
