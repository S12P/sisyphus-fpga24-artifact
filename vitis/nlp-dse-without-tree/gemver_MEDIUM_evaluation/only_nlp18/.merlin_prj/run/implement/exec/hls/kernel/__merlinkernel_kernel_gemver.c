...
#pragma HLS array_partition variable=x_buf_0 complete dim=2
    float A_buf[400][25][16];
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf cyclic factor = 16 dim=3 */
    
#pragma HLS array_partition variable=A_buf complete dim=3
    memcpy_wide_bus_read_float_3d_25_16_512(A_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
{
      memcpy_wide_bus_read_float_512(&u1_10_0_buf[0],(merlin_uint_512 *)u1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_16_512(v1_10_0_buf,0,0,(merlin_uint_512 *)v1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_512(&u2_10_0_buf[0],(merlin_uint_512 *)u2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_16_512(v2_10_0_buf,0,0,(merlin_uint_512 *)v2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL10:
    for (i = 0; i < 400; i++) {
      merlinL9:
...
{
        memcpy_wide_bus_read_float_2d_1_512(x_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)x,(size_t )(i * 16 * 4),sizeof(float ) * ((unsigned long )16),(size_t )16L);
      }
      merlinL6:
...
{
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)x,x_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 16)),sizeof(float ) * ((unsigned long )16),(size_t )16L);
      }
    }
...
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf cyclic factor = 16 dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf_0 cyclic factor = 16 dim=2 */
    memcpy_wide_bus_read_float_2d_16_512(x_buf_0,0,0,(merlin_uint_512 *)x,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
{
...
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf_0 cyclic factor = 16 dim=2 */
    memcpy_wide_bus_write_float_2d_16_512((merlin_uint_512 *)x,x_buf_0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )400),400L);
...
{
      memcpy_wide_bus_read_float_2d_16_512(x_12_0_buf,0,0,(merlin_uint_512 *)x,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL2:
    for (i = 0; i < 400; i++) {
      merlinL1:
...
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf cyclic factor = 16 dim=3 */
    memcpy_wide_bus_write_float_3d_25_16_512((merlin_uint_512 *)A,A_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )160000L),160000L);
  }
...
/* Existing HLS partition: #pragma HLS array_partition variable=v1_10_0_buf cyclic factor = 16 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=v2_10_0_buf cyclic factor = 16 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=x_12_0_buf cyclic factor = 16 dim=2 */
