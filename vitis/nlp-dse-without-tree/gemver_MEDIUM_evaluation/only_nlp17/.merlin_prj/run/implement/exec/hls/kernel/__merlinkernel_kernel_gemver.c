...
#pragma HLS array_partition variable=v2_10_0_buf cyclic factor=16 dim=1
    float u2_10_0_buf[25][16];
    
#pragma HLS array_partition variable=u2_10_0_buf cyclic factor=16 dim=2
    float v1_10_0_buf[400];
    
#pragma HLS array_partition variable=v1_10_0_buf cyclic factor=16 dim=1
    float u1_10_0_buf[25][16];
    
#pragma HLS array_partition variable=u1_10_0_buf cyclic factor=16 dim=2
    float y_11_0_buf[25][16];
    
#pragma HLS array_partition variable=y_11_0_buf complete dim=2
    float A_11_0_buf[25][16][400];
...
{
      memcpy_wide_bus_read_float_2d_16_512(u1_10_0_buf,0,0,(merlin_uint_512 *)u1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_512(&v1_10_0_buf[0],(merlin_uint_512 *)v1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_16_512(u2_10_0_buf,0,0,(merlin_uint_512 *)u2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
...
{
        memcpy_wide_bus_read_float_2d_400_512(A_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)A,(size_t )(((long )i) * 6400L * ((long )4)),sizeof(float ) * ((unsigned long )6400L),(size_t )6400L);
      }
      merlinL9:
      for (int i_sub_1 = 0; i_sub_1 < 16; ++i_sub_1) 
/* Original: #pragma ACCEL PARALLEL FACTOR=16 */
/* Original: #pragma ACCEL PARALLEL FACTOR=16 */
...
{
        memcpy_wide_bus_write_float_2d_400_512((merlin_uint_512 *)A,A_buf,(size_t )0,(size_t )0,(size_t )(((long )4) * (((long )i) * 6400L)),sizeof(float ) * ((unsigned long )6400L),(size_t )6400L);
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf cyclic factor = 16 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf cyclic factor = 16 dim=2 */
{
      memcpy_wide_bus_read_float_3d_16_400_512(A_11_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
{
      memcpy_wide_bus_read_float_2d_16_512(y_11_0_buf,0,0,(merlin_uint_512 *)y,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL7:
    for (i = 0; i < 400; i++) {
      merlinL6:
...
{
      memcpy_wide_bus_read_float_2d_16_512(z_8_0_buf,0,0,(merlin_uint_512 *)z,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL4:
    for (i = 0; i < 25; i++) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
...
#pragma HLS unroll
        x_buf[i * 16 + i_sub_0] = x_buf[i * 16 + i_sub_0] + z_8_0_buf[i][i_sub_0];
      }
    }
{
      memcpy_wide_bus_read_float_3d_16_400_512(A_12_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
    merlinL2:
    for (i = 0; i < 25; i++) 
/* Original: #pragma ACCEL PARALLEL FACTOR=16 */
...
{
        memcpy_wide_bus_read_float_2d_1_512(w_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)w,(size_t )(i * 16 * 4),sizeof(float ) * ((unsigned long )16),(size_t )16L);
      }
      merlinL1:
...
{
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)w,w_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 16)),sizeof(float ) * ((unsigned long )16),(size_t )16L);
      }
    }
...
/* Existing HLS partition: #pragma HLS array_partition variable=u1_10_0_buf cyclic factor = 16 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=u2_10_0_buf cyclic factor = 16 dim=2 */
...
/* Existing HLS partition: #pragma HLS array_partition variable=y_11_0_buf cyclic factor = 16 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=z_8_0_buf cyclic factor = 16 dim=2 */
}
