...
void kernel_gemver(float alpha,float beta,merlin_uint_256 A[20000],merlin_uint_512 u1[25],merlin_uint_256 v1[50],merlin_uint_512 u2[25],merlin_uint_256 v2[50],merlin_uint_512 w[25],merlin_uint_256 x[50],merlin_uint_512 y[25],merlin_uint_256 z[50])
{
  
#pragma HLS INTERFACE m_axi port=A offset=slave depth=20000 bundle=merlin_gmem_kernel_gemver_256_A
...
#pragma HLS INTERFACE m_axi port=v1 offset=slave depth=50 bundle=merlin_gmem_kernel_gemver_256_0
  
#pragma HLS INTERFACE m_axi port=v2 offset=slave depth=50 bundle=merlin_gmem_kernel_gemver_256_1
  
#pragma HLS INTERFACE m_axi port=w offset=slave depth=25 bundle=merlin_gmem_kernel_gemver_512_w
  
#pragma HLS INTERFACE m_axi port=x offset=slave depth=50 bundle=merlin_gmem_kernel_gemver_256_x
  
#pragma HLS INTERFACE m_axi port=y offset=slave depth=25 bundle=merlin_gmem_kernel_gemver_512_2
  
#pragma HLS INTERFACE m_axi port=z offset=slave depth=50 bundle=merlin_gmem_kernel_gemver_256_2
...
#pragma HLS array_partition variable=A_buf complete dim=3
    memcpy_wide_bus_read_float_3d_2_200_256(A_buf,0,0,0,(merlin_uint_256 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
{
      memcpy_wide_bus_read_float_512(&u1_10_0_buf[0],(merlin_uint_512 *)u1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_200_256(v1_10_0_buf,0,0,(merlin_uint_256 *)v1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_512(&u2_10_0_buf[0],(merlin_uint_512 *)u2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_200_256(v2_10_0_buf,0,0,(merlin_uint_256 *)v2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL10:
    for (i = 0; i < 400; i++) {
      merlinL9:
...
#pragma HLS pipeline
        merlinL8:
        for (int j_sub_0 = 0; j_sub_0 < 200; ++j_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=200 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=200 */
...
{
        memcpy_wide_bus_read_float_2d_1_256(x_buf,(size_t )0,(size_t )0,(merlin_uint_256 *)x,(size_t )(i * 200 * 4),sizeof(float ) * ((unsigned long )200),(size_t )200L);
      }
      merlinL6:
...
{
        memcpy_wide_bus_write_float_2d_1_256((merlin_uint_256 *)x,x_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 200)),sizeof(float ) * ((unsigned long )200),(size_t )200L);
      }
    }
...
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf cyclic factor = 8 dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf_0 cyclic factor = 8 dim=2 */
    memcpy_wide_bus_read_float_2d_200_256(x_buf_0,0,0,(merlin_uint_256 *)x,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
{
...
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf_0 cyclic factor = 8 dim=2 */
    memcpy_wide_bus_write_float_2d_200_256((merlin_uint_256 *)x,x_buf_0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )400),400L);
...
{
      memcpy_wide_bus_read_float_2d_200_256(x_12_0_buf,0,0,(merlin_uint_256 *)x,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL2:
    for (i = 0; i < 400; i++) {
      merlinL1:
...
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf cyclic factor = 8 dim=3 */
    memcpy_wide_bus_write_float_3d_2_200_256((merlin_uint_256 *)A,A_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )160000L),160000L);
  }
...
/* Existing HLS partition: #pragma HLS array_partition variable=v1_10_0_buf cyclic factor = 8 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=v2_10_0_buf cyclic factor = 8 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=x_12_0_buf cyclic factor = 8 dim=2 */
