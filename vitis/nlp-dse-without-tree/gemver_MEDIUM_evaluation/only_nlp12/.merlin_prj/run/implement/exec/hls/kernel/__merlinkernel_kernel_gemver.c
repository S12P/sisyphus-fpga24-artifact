...
void kernel_gemver(float alpha,float beta,merlin_uint_128 A[40000],merlin_uint_512 u1[25],merlin_uint_128 v1[100],merlin_uint_512 u2[25],merlin_uint_128 v2[100],merlin_uint_512 w[25],merlin_uint_128 x[100],merlin_uint_512 y[25],merlin_uint_128 z[100])
{
  
#pragma HLS INTERFACE m_axi port=A offset=slave depth=40000 bundle=merlin_gmem_kernel_gemver_128_A
...
#pragma HLS INTERFACE m_axi port=v1 offset=slave depth=100 bundle=merlin_gmem_kernel_gemver_128_0
  
#pragma HLS INTERFACE m_axi port=v2 offset=slave depth=100 bundle=merlin_gmem_kernel_gemver_128_1
  
#pragma HLS INTERFACE m_axi port=w offset=slave depth=25 bundle=merlin_gmem_kernel_gemver_512_w
  
#pragma HLS INTERFACE m_axi port=x offset=slave depth=100 bundle=merlin_gmem_kernel_gemver_128_x
  
#pragma HLS INTERFACE m_axi port=y offset=slave depth=25 bundle=merlin_gmem_kernel_gemver_512_2
  
#pragma HLS INTERFACE m_axi port=z offset=slave depth=100 bundle=merlin_gmem_kernel_gemver_128_2
...
#pragma HLS array_partition variable=A_buf complete dim=3
    memcpy_wide_bus_read_float_3d_4_100_128(A_buf,0,0,0,(merlin_uint_128 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
{
      memcpy_wide_bus_read_float_512(&u1_10_0_buf[0],(merlin_uint_512 *)u1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_100_128(v1_10_0_buf,0,0,(merlin_uint_128 *)v1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_512(&u2_10_0_buf[0],(merlin_uint_512 *)u2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
{
      memcpy_wide_bus_read_float_2d_100_128(v2_10_0_buf,0,0,(merlin_uint_128 *)v2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL10:
    for (i = 0; i < 400; i++) {
      merlinL9:
...
#pragma HLS pipeline
        merlinL8:
        for (int j_sub_0 = 0; j_sub_0 < 100; ++j_sub_0) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=100 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=100 */
...
{
        memcpy_wide_bus_read_float_2d_1_128(x_buf,(size_t )0,(size_t )0,(merlin_uint_128 *)x,(size_t )(i * 100 * 4),sizeof(float ) * ((unsigned long )100),(size_t )100L);
      }
      merlinL6:
...
{
        memcpy_wide_bus_write_float_2d_1_128((merlin_uint_128 *)x,x_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 100)),sizeof(float ) * ((unsigned long )100),(size_t )100L);
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf cyclic factor = 4 dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf cyclic factor = 4 dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf_0 cyclic factor = 4 dim=2 */
    memcpy_wide_bus_read_float_2d_100_128(x_buf_0,0,0,(merlin_uint_128 *)x,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
{
      memcpy_wide_bus_read_float_2d_100_128(z_8_0_buf,0,0,(merlin_uint_128 *)z,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL4:
    for (i = 0; i < 4; i++) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
...
/* Existing HLS partition: #pragma HLS array_partition variable=x_buf_0 cyclic factor = 4 dim=2 */
    memcpy_wide_bus_write_float_2d_100_128((merlin_uint_128 *)x,x_buf_0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )400),400L);
...
{
      memcpy_wide_bus_read_float_2d_100_128(x_12_0_buf,0,0,(merlin_uint_128 *)x,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL2:
    for (i = 0; i < 400; i++) {
      merlinL1:
...
#pragma HLS pipeline
        merlinL0:
        for (int j_sub = 0; j_sub < 100; ++j_sub) 
/* Original: #pragma ACCEL PIPELINE FLATTEN */
/* Original: #pragma ACCEL PARALLEL FACTOR=100 */
/* Original: #pragma ACCEL PIPELINE */
/* Original: #pragma ACCEL PARALLEL FACTOR=100 */
...
/* Existing HLS partition: #pragma HLS array_partition variable=A_buf cyclic factor = 4 dim=3 */
    memcpy_wide_bus_write_float_3d_4_100_128((merlin_uint_128 *)A,A_buf,0,0,0,(4 * 0),sizeof(float ) * ((unsigned long )160000L),160000L);
  }
...
/* Existing HLS partition: #pragma HLS array_partition variable=v1_10_0_buf cyclic factor = 4 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=v2_10_0_buf cyclic factor = 4 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=x_12_0_buf cyclic factor = 4 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_11_0_buf cyclic factor = 16 dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=z_8_0_buf cyclic factor = 4 dim=2 */
}
