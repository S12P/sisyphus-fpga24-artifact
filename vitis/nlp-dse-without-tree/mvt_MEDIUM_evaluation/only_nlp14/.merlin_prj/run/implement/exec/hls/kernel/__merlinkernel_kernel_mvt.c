...
{
      memcpy_wide_bus_read_float_3d_16_400_512(A_6_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
{
...
{
        memcpy_wide_bus_read_float_2d_1_512(x1_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)x1,(size_t )(i * 16 * 4),sizeof(float ) * ((unsigned long )16),(size_t )16L);
      }
      merlinL4:
...
{
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)x1,x1_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 16)),sizeof(float ) * ((unsigned long )16),(size_t )16L);
      }
    }
...
{
      memcpy_wide_bus_read_float_3d_16_400_512(A_7_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
{
      memcpy_wide_bus_read_float_2d_16_512(y_2_7_0_buf,0,0,(merlin_uint_512 *)y_2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL2:
    for (i = 0; i < 400; i++) {
      merlinL1:
...
/* Existing HLS partition: #pragma HLS array_partition variable=y_2_7_0_buf cyclic factor = 16 dim=2 */
}
