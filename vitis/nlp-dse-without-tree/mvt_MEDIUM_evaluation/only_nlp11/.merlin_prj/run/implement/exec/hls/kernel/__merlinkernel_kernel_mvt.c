...
void kernel_mvt_L_0_0_para(int j,float A_6_0_buf[50][400],float x1_buf[50][1],float y_1_6_0_buf_dup[50][400])
{
  
#pragma HLS inline off
  int i_sub;
  merlinL0:
  for (i_sub = 0; i_sub < 50; ++i_sub) {
...
void kernel_mvt(merlin_uint_512 x1[25],merlin_uint_512 x2[25],merlin_uint_512 y_1[25],merlin_uint_64 y_2[200],merlin_uint_512 A[10000])
{
...
#pragma HLS INTERFACE m_axi port=y_2 offset=slave depth=200 bundle=merlin_gmem_kernel_mvt_64_0
...
#pragma HLS pipeline
        merlinL7:
        for (_i0 = ((long )0); ((unsigned long )_i0) <= 49UL; ++_i0) {
...
{
        memcpy_wide_bus_read_float_2d_1_512(x1_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)x1,(size_t )(i * 50 * 4),sizeof(float ) * ((unsigned long )50),(size_t )50L);
      }
{
        memcpy_wide_bus_read_float_2d_400_512(A_6_0_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)A,(size_t )(((long )i) * 20000L * ((long )4)),sizeof(float ) * ((unsigned long )20000L),(size_t )20000L);
      }
...
/* Original: #pragma ACCEL PARALLEL */
{
        ...
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)x1,x1_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 50)),sizeof(float ) * ((unsigned long )50),(size_t )50L);
      }
    }
...
{
      memcpy_wide_bus_read_float_3d_50_400_512(A_7_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
{
      memcpy_wide_bus_read_float_2d_50_64(y_2_7_0_buf,0,0,(merlin_uint_64 *)y_2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL4:
    for (i = 0; i < 400; i++) {
      merlinL3:
...
/* Existing HLS partition: #pragma HLS array_partition variable=y_2_7_0_buf cyclic factor = 2 dim=2 */
}
