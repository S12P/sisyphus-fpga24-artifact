...
/* Existing HLS partition: #pragma HLS array_partition variable=x1_buf cyclic factor = 4 dim=1 */
    
#pragma HLS array_partition variable=x1_buf complete dim=2
    
#pragma HLS array_partition variable=x1_buf cyclic factor=4 dim=1
    memcpy_wide_bus_read_float_2d_4_512(x1_buf,0,0,(merlin_uint_512 *)x1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
{
      memcpy_wide_bus_read_float_4d_4_25_16_512(A_8_0_buf,0,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
{
      memcpy_wide_bus_read_float_2d_16_512(y_1_8_0_buf,0,0,(merlin_uint_512 *)y_1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL7:
    for (i = 0; i < 100; i++) 
...
/* Existing HLS partition: #pragma HLS array_partition variable=x1_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=x1_buf cyclic factor = 4 dim=1 */
    memcpy_wide_bus_write_float_2d_4_512((merlin_uint_512 *)x1,x1_buf,0,0,(4 * 0),sizeof(float ) * ((unsigned long )400),400L);
/* Existing HLS partition: #pragma HLS array_partition variable=x2_buf cyclic factor = 16 dim=2 */
    memcpy_wide_bus_read_float_2d_16_512(x2_buf,0,0,(merlin_uint_512 *)x2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
{
      memcpy_wide_bus_read_float_4d_4_25_16_512(A_9_0_buf,0,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
{
      memcpy_wide_bus_read_float_2d_4_512(y_2_9_0_buf,0,0,(merlin_uint_512 *)y_2,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL3:
    for (i = 0; i < 25; i++) 
/* Original: #pragma ACCEL PARALLEL FACTOR=16 */
...
/* Existing HLS partition: #pragma HLS array_partition variable=x2_buf cyclic factor = 16 dim=2 */
    memcpy_wide_bus_write_float_2d_16_512((merlin_uint_512 *)x2,x2_buf,0,0,(4 * 0),sizeof(float ) * ((unsigned long )400),400L);
  }
/* Existing HLS partition: #pragma HLS array_partition variable=A_8_0_buf cyclic factor = 16 dim=4 */
/* Existing HLS partition: #pragma HLS array_partition variable=A_9_0_buf cyclic factor = 16 dim=4 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_1_8_0_buf cyclic factor = 16 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_2_9_0_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_2_9_0_buf cyclic factor = 4 dim=1 */
}
