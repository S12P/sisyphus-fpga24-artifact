...
#pragma HLS array_partition variable=y_1_6_0_buf complete dim=1
    float A_6_0_buf[200][2][400];
...
/* Existing HLS partition: #pragma HLS array_partition variable=x1_buf cyclic factor = 8 dim=1 */
    
#pragma HLS array_partition variable=x1_buf complete dim=2
    
#pragma HLS array_partition variable=x1_buf cyclic factor=8 dim=1
    memcpy_wide_bus_read_float_2d_2_512(x1_buf,0,0,(merlin_uint_512 *)x1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
{
      memcpy_wide_bus_read_float_3d_2_400_512(A_6_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
{
...
#pragma HLS unroll
          x1_buf[i][i_sub_0] = x1_buf[i][i_sub_0] + A_6_0_buf[i][i_sub_0][j] * y_1_6_0_buf[j];
        }
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=x1_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=x1_buf cyclic factor = 8 dim=1 */
    memcpy_wide_bus_write_float_2d_2_512((merlin_uint_512 *)x1,x1_buf,0,0,(4 * 0),sizeof(float ) * ((unsigned long )400),400L);
