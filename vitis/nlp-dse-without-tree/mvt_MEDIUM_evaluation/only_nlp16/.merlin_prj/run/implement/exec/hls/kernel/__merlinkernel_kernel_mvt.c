...
#pragma ACCEL interface variable=x1 depth=400 max_depth=400
  ...
  int j;
{
    float y_1_6_0_buf[50][8];
    
#pragma HLS array_partition variable=y_1_6_0_buf complete dim=2
    
#pragma HLS array_partition variable=y_1_6_0_buf cyclic factor=2 dim=1
    float A_6_0_buf[400][50][8];
...
#pragma HLS array_partition variable=y_2_7_0_buf cyclic factor=16 dim=1
    float A_7_0_buf[400][50][8];
    
#pragma HLS array_partition variable=A_7_0_buf complete dim=3
    
#pragma HLS array_partition variable=A_7_0_buf cyclic factor=2 dim=2
    float x1_buf[400];
...
{
      memcpy_wide_bus_read_float_3d_50_8_512(A_6_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
{
      memcpy_wide_bus_read_float_2d_8_512(y_1_6_0_buf,0,0,(merlin_uint_512 *)y_1,(0 * 4),sizeof(float ) * ((unsigned long )400),400L);
    }
    merlinL5:
    for (i = 0; i < 400; i++) {
      merlinL4:
...
{
      memcpy_wide_bus_read_float_3d_50_8_512(A_7_0_buf,0,0,0,(merlin_uint_512 *)A,(0 * 4),sizeof(float ) * ((unsigned long )160000L),160000L);
    }
{
...
{
        memcpy_wide_bus_read_float_2d_1_512(x2_buf,(size_t )0,(size_t )0,(merlin_uint_512 *)x2,(size_t )(i * 8 * 4),sizeof(float ) * ((unsigned long )8),(size_t )8L);
      }
      merlinL1:
...
{
        memcpy_wide_bus_write_float_2d_1_512((merlin_uint_512 *)x2,x2_buf,(size_t )0,(size_t )0,(size_t )(4 * (i * 8)),sizeof(float ) * ((unsigned long )8),(size_t )8L);
      }
    }
/* Existing HLS partition: #pragma HLS array_partition variable=x2_buf complete dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=x2_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=x2_buf complete dim=1 */
/* Existing HLS partition: #pragma HLS array_partition variable=x2_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=x2_buf cyclic factor = 2 dim=0 */
/* Existing HLS partition: #pragma HLS array_partition variable=x2_buf cyclic factor = 2 dim=0 */
  }
...
/* Existing HLS partition: #pragma HLS array_partition variable=A_7_0_buf cyclic factor = 2 dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_1_6_0_buf complete dim=2 */
/* Existing HLS partition: #pragma HLS array_partition variable=y_1_6_0_buf cyclic factor = 2 dim=1 */
