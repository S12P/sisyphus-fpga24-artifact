...
#pragma ACCEL interface variable=C depth=100,64 max_depth=100,64
  ...
  int j;
  int k;
  memcpy((void *)(&C_buf[0]),(const void *)(&C[0]),sizeof(float ) * 6400L);
{
    memcpy((void *)(&A_5_0_buf[0]),(const void *)(&A[0]),sizeof(float ) * 10000L);
  }
{
    memcpy((void *)(&B_5_0_buf[0]),(const void *)(&B[0]),sizeof(float ) * 6400L);
  }
  for (i = 0; i < 100; i++) 
...
#pragma ACCEL PARALLEL COMPLETE
          C_buf[i * 64L + j * 8 + j_sub] += A_5_0_buf[i * 100L + k] * B_5_0_buf[j * 8 + k * 64L + j_sub];
        }
      }
    }
  }
  memcpy((void *)(&C[0]),(const void *)(&C_buf[0]),sizeof(float ) * 6400L);
}
