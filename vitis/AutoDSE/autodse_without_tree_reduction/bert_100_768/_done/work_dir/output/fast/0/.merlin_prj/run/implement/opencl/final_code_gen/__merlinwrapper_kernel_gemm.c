...
#pragma ACCEL wrapper VARIABLE=C port=C data_type=float depth=100,768 max_depth=100,768 io=RW continue=1 copy=true
    
#pragma ACCEL wrapper VARIABLE=A port=A data_type=float depth=100,100 max_depth=100,100 io=RW continue=1 copy=true
    
#pragma ACCEL wrapper VARIABLE=B port=B data_type=float depth=100,768 max_depth=100,768 io=RW continue=1 copy=true
  }
...
{
/*
    static float __m_C[76800];
    static float __m_A[10000];
    static float __m_B[76800];
*/
{
      
#pragma ACCEL debug printf("[Merlin Info] Start %s data for %s, data size = %s...\n", "copy in", "C", "100 * 768 * sizeof(float )");
      
#pragma ACCEL debug fflush(stdout);
      if (C != 0) {
        __CMOST_COMMENT__ __kernel_gemm__C_buffer;
        opencl_write_buffer(__kernel_gemm__C_buffer,commands[0],0 * sizeof(float ),(float *)C[0],((unsigned long )(100 * 768)) * sizeof(float ));
      }
       else {
...
#pragma ACCEL debug fflush(stdout);
      if (A != 0) {
        __CMOST_COMMENT__ __kernel_gemm__A_buffer;
...
#pragma ACCEL debug printf("[Merlin Info] Start %s data for %s, data size = %s...\n", "copy in", "B", "100 * 768 * sizeof(float )");
      
#pragma ACCEL debug fflush(stdout);
      if (B != 0) {
        __CMOST_COMMENT__ __kernel_gemm__B_buffer;
        opencl_write_buffer(__kernel_gemm__B_buffer,commands[0],0 * sizeof(float ),(float *)B[0],((unsigned long )(100 * 768)) * sizeof(float ));
      }
       else {
...
#pragma ACCEL debug printf("[Merlin Info] Start %s data for %s, data size = %s...\n", "copy out", "C", "100 * 768 * sizeof(float )");
      
#pragma ACCEL debug fflush(stdout);
      if (C != 0) {
        __CMOST_COMMENT__ __kernel_gemm__C_buffer;
        opencl_read_buffer(__kernel_gemm__C_buffer,commands[0],0 * sizeof(float ),(float *)C[0],((unsigned long )(100 * 768)) * sizeof(float ));
      }
       else {
        printf("Error! Detected null pointer 'C' for external memory.\n");
        exit(1);
...
#pragma ACCEL debug printf("[Merlin Info] Start %s data for %s, data size = %s...\n", "copy out", "B", "100 * 768 * sizeof(float )");
      
#pragma ACCEL debug fflush(stdout);
      if (B != 0) {
        __CMOST_COMMENT__ __kernel_gemm__B_buffer;
        opencl_read_buffer(__kernel_gemm__B_buffer,commands[0],0 * sizeof(float ),(float *)B[0],((unsigned long )(100 * 768)) * sizeof(float ));
      }
       else {
        printf("Error! Detected null pointer 'B' for external memory.\n");
        exit(1);
...
#pragma ACCEL debug fflush(stdout);
    if (C != 0) {
      __CMOST_COMMENT__ __kernel_gemm__C_buffer;
      __CMOST_COMMENT__ __event___kernel_gemm__C_buffer;
/* __MERLIN_EVENT_WRITE__ __event___kernel_gemm__C_buffer */
      int __MERLIN_EVENT_WRITE____event___kernel_gemm__C_buffer;
      opencl_write_buffer_nb(__kernel_gemm__C_buffer,commands[0],0 * sizeof(float ),(float *)C[0],((unsigned long )(100 * 768)) * sizeof(float ),&__event___kernel_gemm__C_buffer);
    }
     else {
      printf("Error! Detected null pointer 'C' for external memory.\n");
      exit(1);
...
#pragma ACCEL debug fflush(stdout);
    if (A != 0) {
      __CMOST_COMMENT__ __kernel_gemm__A_buffer;
      __CMOST_COMMENT__ __event___kernel_gemm__A_buffer;
...
#pragma ACCEL debug fflush(stdout);
    if (B != 0) {
      __CMOST_COMMENT__ __kernel_gemm__B_buffer;
      __CMOST_COMMENT__ __event___kernel_gemm__B_buffer;
/* __MERLIN_EVENT_WRITE__ __event___kernel_gemm__B_buffer */
      int __MERLIN_EVENT_WRITE____event___kernel_gemm__B_buffer;
      opencl_write_buffer_nb(__kernel_gemm__B_buffer,commands[0],0 * sizeof(float ),(float *)B[0],((unsigned long )(100 * 768)) * sizeof(float ),&__event___kernel_gemm__B_buffer);
    }
     else {
      printf("Error! Detected null pointer 'B' for external memory.\n");
      exit(1);
...
/* __MERLIN_EVENT_READ__ __event___kernel_gemm__C_buffer */
      ...
      opencl_read_buffer_nb(__kernel_gemm__C_buffer,commands[0],0 * sizeof(float ),(float *)C[0],((unsigned long )(100 * 768)) * sizeof(float ),&__event___kernel_gemm__C_buffer);
    }
     else {
      printf("Error! Detected null pointer 'C' for external memory.\n");
      exit(1);
...
/* __MERLIN_EVENT_READ__ __event___kernel_gemm__B_buffer */
      ...
      opencl_read_buffer_nb(__kernel_gemm__B_buffer,commands[0],0 * sizeof(float ),(float *)B[0],((unsigned long )(100 * 768)) * sizeof(float ),&__event___kernel_gemm__B_buffer);
    }
     else {
      printf("Error! Detected null pointer 'B' for external memory.\n");
      exit(1);
