...
void task2(float r[410], float s[390], float A[410][390], float2 vA[79950], float2 vs[195], float2 vq[205], float2 vp[195], float2 vr[205]) {
    int j;
    int i;
...
void task3(float p[390], float A[410][390], float q[410], float2 vA[79950], float2 vs[195], float2 vq[205], float2 vp[195], float2 vr[205]) {
    int i;
    int j;
    for (int i0 = 0; i0 < 1; i0++) {
        for (int j0 = 0; j0 < 1; j0++) {
...
#pragma HLS ARRAY_PARTITION variable=q cyclic factor=410 dim=1
    ...
    task2(r, s, A, vA, vs, vq, vp, vr);
    task3(p, A, q, vA, vs, vq, vp, vr);
    store_s(s, vs);
    store_q(q, vq);
}
