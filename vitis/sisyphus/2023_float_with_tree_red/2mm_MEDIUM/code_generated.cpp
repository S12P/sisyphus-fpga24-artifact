...
void task1(float A[180][210], float tmp[180][190], float B[210][190], float alpha, float beta, float2 vtmp[17100], float2 vA[18900], float2 vB[19950], float4 vC[10450], float4 vD[9900]) {
    int k;
    int j;
    int i;
...
void task3(float tmp[180][190], float D[180][220], float C[190][220], float alpha, float beta, float2 vtmp[17100], float2 vA[18900], float2 vB[19950], float4 vC[10450], float4 vD[9900]) {
    int i;
    int k;
    int j;
    for (int i0 = 0; i0 < 1; i0++) {
...
#pragma HLS ARRAY_PARTITION variable=C cyclic factor=220 dim=2
    ...
    task3(tmp, D, C, alpha, beta, vtmp, vA, vB, vC, vD);
    store_tmp(tmp, vtmp);
    store_D(D, vD);
}
