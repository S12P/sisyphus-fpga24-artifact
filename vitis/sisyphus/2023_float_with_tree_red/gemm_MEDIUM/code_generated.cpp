...
void task1(float A[200][240], float C[200][220], float B[240][220], float alpha, float beta, float4 vC[11000], float16 vA[3000], float4 vB[13200]) {
    int j;
    int k;
    int i;
...
#pragma HLS ARRAY_PARTITION variable=B cyclic factor=200 dim=2

    load_C(C, vC);
    load_A(A, vA);
    load_B(B, vB);
    task0(C, alpha, beta, vC, vA, vB);
    task1(A, C, B, alpha, beta, vC, vA, vB);
    store_C(C, vC);
}
