...
void task3(float y[250], float x[250], float B[250][250], float alpha, float beta, float2 vA[31250], float2 vB[31250], float2 vtmp[125], float2 vx[125], float2 vy[125]) {
    int j;
    int i;
    for (int j0 = 0; j0 < 1; j0++) {
        for (int i0 = 0; i0 < 1; i0++) {
...
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=250 dim=2
    ...
    task3(y, x, B, alpha, beta, vA, vB, vtmp, vx, vy);
