...
void task1(float C[240][240], float A[240][200], float B[240][200], float alpha, float beta, float16 vC[3600], float8 vA[6000], float8 vB[6000]) {
    int i;
    int k;
    int j;
    for (int i0 = 0; i0 < 1; i0++) {
...
#pragma HLS ARRAY_PARTITION variable=C cyclic factor=48 dim=2
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=48 dim=1
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=8 dim=2
#pragma HLS ARRAY_PARTITION variable=B cyclic factor=48 dim=1
#pragma HLS ARRAY_PARTITION variable=B cyclic factor=8 dim=2
    ...
    task1(C, A, B, alpha, beta, vC, vA, vB);
    store_C(C, vC);
}
