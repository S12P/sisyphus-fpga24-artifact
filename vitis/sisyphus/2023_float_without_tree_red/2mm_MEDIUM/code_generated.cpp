...
void task3(float C[190][220], float D[180][220], float tmp[180][190], float alpha, float beta, float2 vtmp[17100], float2 vA[18900], float2 vB[19950], float4 vC[10450], float4 vD[9900]) {
    int i;
    int k;
    int j;
    for (int i0 = 0; i0 < 1; i0++) {
...
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=180 dim=1
...
#pragma HLS ARRAY_PARTITION variable=tmp cyclic factor=180 dim=1
#pragma HLS ARRAY_PARTITION variable=tmp cyclic factor=2 dim=2
    ...
    task3(C, D, tmp, alpha, beta, vtmp, vA, vB, vC, vD);
    store_tmp(tmp, vtmp);
    store_D(D, vD);
}
