...
void task5(float F[190][210], float E[180][190], float G[180][210], float2 vE[17100], float8 vA[4500], float2 vB[19000], float2 vF[19950], float4 vC[10450], float2 vD[23100], float2 vG[18900]) {
    int i;
    int k;
    int j;
    for (int i0 = 0; i0 < 1; i0++) {
...
#pragma HLS ARRAY_PARTITION variable=F cyclic factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=F cyclic factor=210 dim=2
#pragma HLS ARRAY_PARTITION variable=E cyclic factor=5 dim=1
#pragma HLS ARRAY_PARTITION variable=E cyclic factor=190 dim=2
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=4 dim=1
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=10 dim=2
#pragma HLS ARRAY_PARTITION variable=D cyclic factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=D cyclic factor=210 dim=2
#pragma HLS ARRAY_PARTITION variable=G cyclic factor=4 dim=1
#pragma HLS ARRAY_PARTITION variable=G cyclic factor=210 dim=2
#pragma HLS ARRAY_PARTITION variable=B cyclic factor=5 dim=1
#pragma HLS ARRAY_PARTITION variable=B cyclic factor=190 dim=2
    ...
    task5(F, E, G, vE, vA, vB, vF, vC, vD, vG);
    store_E(E, vE);
    store_F(F, vF);
    store_G(G, vG);
}
