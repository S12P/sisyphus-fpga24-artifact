...
void task1(float A[240][200], float C[240][240], float alpha, float beta, float16 vC[3600], float8 vA[6000]) {
    int i;
    int k;
    int j;
    for (int i0 = 0; i0 < 1; i0++) {
...
//                                C[i][j] +=alpha * A[i][k] * A[j][k] ;
                                C[i][j] = compute_operation_task1(C[i][j], C[i][j], alpha, A[i][k], A[j][k], j, i);
                             }
                        }
                    }
                }
...
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=120 dim=1
