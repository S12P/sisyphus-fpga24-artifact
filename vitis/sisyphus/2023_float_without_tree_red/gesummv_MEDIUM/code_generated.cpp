...
void task2(float A[250][250], float tmp[250], float x[250], float alpha, float beta, float2 vA[31250], float2 vB[31250], float2 vtmp[125], float2 vx[125], float2 vy[125]) {
    int i;
    int j;
    for (int i0 = 0; i0 < 1; i0++) {
        for (int j0 = 0; j0 < 1; j0++) {
            for (int j1 = 0; j1 < 250; j1++) {
#pragma HLS pipeline
...
void task4(float tmp[250], float y[250], float alpha, float beta, float2 vA[31250], float2 vB[31250], float2 vtmp[125], float2 vx[125], float2 vy[125]) {
    int i;
    for (int i0 = 0; i0 < 1; i0++) {
        for (int i1 = 0; i1 < 2; i1++) {
#pragma HLS pipeline
...
#pragma HLS ARRAY_PARTITION variable=B cyclic factor=250 dim=1
#pragma HLS ARRAY_PARTITION variable=B cyclic factor=2 dim=2
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=250 dim=1
...
#pragma HLS ARRAY_PARTITION variable=x cyclic factor=2 dim=1
    ...
    task2(A, tmp, x, alpha, beta, vA, vB, vtmp, vx, vy);
    task3(B, y, x, alpha, beta, vA, vB, vtmp, vx, vy);
    task4(tmp, y, alpha, beta, vA, vB, vtmp, vx, vy);
    store_tmp(tmp, vtmp);
    store_y(y, vy);
}
