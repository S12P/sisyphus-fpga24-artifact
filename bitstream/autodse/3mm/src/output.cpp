...
#pragma HLS pipeline
          merlinL7:
          for (int k_sub_0 = 0; k_sub_0 < 8; ++k_sub_0) 
/* Original: #pragma ACCEL PARALLEL FACTOR=8 */
/* Original: #pragma ACCEL PARALLEL FACTOR=8 */
...
#pragma HLS unroll
            E_buf[i][j] += A_11_0_buf[i][k][k_sub_0] * B_11_0_buf[k][k_sub_0][j];
          }
        }
      }
    }
...
#pragma HLS unroll
            G_buf[i][j] += E_buf[i][((long )k) * 2L + ((long )k_sub)] * F_buf[((long )k) * 2L + ((long )k_sub)][j];
          }
        }
      }
    }
