...
void kernel_3mm_ori(float E[180][190],float A[180][200],float B[200][190],float F[190][210],float C[190][220],float D[220][210],float G[180][210])
{
  int i;
  int j;
  int k;
...
int main(){
    ...
    kernel_3mm_ori(E_ori, A_ori, B_ori, F_ori, C_ori, D_ori, G_ori);
    ...
    for(int i0 = 0; i0 < 180; i0++){
        for(int i1 = 0; i1 < 210; i1++){
            if(abs(G_ori[i0][i1] - G_new[i0][i1]) > 0.0001){
                printf("Error in GG[%d][%d]...%f %f\n", i0, i1, G_ori[i0][i1], G_new[i0][i1]);
                return 1;
            }
        }
    }
