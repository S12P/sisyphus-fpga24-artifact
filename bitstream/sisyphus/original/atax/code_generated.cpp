...
void task2(float A[390][410], float x[410], float tmp[390], float2 vA[79950], float2 vx[205], float2 vy[205], float2 vtmp[195]) {
    int i;
    int j;
    for (int i0 = 0; i0 < 1; i0++) {
...
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=6 dim=1
#pragma HLS ARRAY_PARTITION variable=A cyclic factor=82 dim=2
#pragma HLS ARRAY_PARTITION variable=x cyclic factor=82 dim=1
