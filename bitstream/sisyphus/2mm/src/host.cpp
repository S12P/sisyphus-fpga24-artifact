...
int main(int argc, char* argv[]) {
    ...
    float tmp_new_0[180 * 190];
    float A_ori[180][210];
    float A_new_0[180 * 220];
    float B_ori[210][190];
    float B_new_0[210 * 190];
    float C_ori[190][220];
    float C_new_0[190 * 220];
    float D_ori[180][220];
    float D_new_0[180 * 220];
    int memIndex = 0;
    float val;
    ...
    cl::Buffer tmp_0NewOCL =
        cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                   sizeof(float) * 180 * 190, tmp_new_0, &err);
    if (err != CL_SUCCESS) {
    ...
    cl::Buffer A_0NewOCL =
        cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                   sizeof(float) * 180 * 220, A_new_0, &err);
    if (err != CL_SUCCESS) {
    ...
    cl::Buffer B_0NewOCL =
        cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                   sizeof(float) * 210 * 190, B_new_0, &err);
    if (err != CL_SUCCESS) {
